...
func (c *staticCatalog) List() []ProviderModel {
	...
	for i := range out {
		...
		if out[i].Capabilities != nil {
			caps := CloneProviderCaps(*out[i].Capabilities)
...
func (c *staticCatalog) Get(modelID string) *ProviderModel {
	...
	if mm.Capabilities != nil {
		caps := CloneProviderCaps(*mm.Capabilities)
...
func aggregateCapabilities(models []ProviderModel, fallback ProviderCaps) ProviderCaps {
	...
	auth := fallback.Auth
	auth.CredentialKinds = append([]CredentialKind(nil), auth.CredentialKinds...)
	quota := fallback.Quota
	classification := fallback.Classification
	classification.AuthMarkers = append([]string(nil), classification.AuthMarkers...)
	classification.EntitlementMarkers = append([]string(nil), classification.EntitlementMarkers...)
	classification.QuotaMarkers = append([]string(nil), classification.QuotaMarkers...)
	classification.CapacityMarkers = append([]string(nil), classification.CapacityMarkers...)
	...
	for _, m := range models {
		...
		if c.Auth.Refreshable {
			auth.Refreshable = true
		}
		if c.Auth.AccountScoped {
			auth.AccountScoped = true
		}
		auth.CredentialKinds = append(auth.CredentialKinds, c.Auth.CredentialKinds...)
		if c.Quota.Required {
			quota.Required = true
		}
		if c.Quota.AccountScoped {
			quota.AccountScoped = true
		}
		if c.Quota.UsageTracked {
			quota.UsageTracked = true
		}
		classification.AuthMarkers = append(classification.AuthMarkers, c.Classification.AuthMarkers...)
		classification.EntitlementMarkers = append(classification.EntitlementMarkers, c.Classification.EntitlementMarkers...)
		classification.QuotaMarkers = append(classification.QuotaMarkers, c.Classification.QuotaMarkers...)
		classification.CapacityMarkers = append(classification.CapacityMarkers, c.Classification.CapacityMarkers...)
		if classification.AuthScope == "" {
			classification.AuthScope = c.Classification.AuthScope
		}
		if classification.EntitlementScope == "" {
			classification.EntitlementScope = c.Classification.EntitlementScope
		}
		if classification.QuotaScope == "" {
			classification.QuotaScope = c.Classification.QuotaScope
		}
		if classification.CapacityScope == "" {
			classification.CapacityScope = c.Classification.CapacityScope
		}
	...
	merged.Auth = auth
	auth.CredentialKinds = DedupeCredentialKinds(auth.CredentialKinds)
	merged.Auth = auth
	merged.Quota = quota
	classification.AuthMarkers = DedupeStrings(classification.AuthMarkers)
	classification.EntitlementMarkers = DedupeStrings(classification.EntitlementMarkers)
	classification.QuotaMarkers = DedupeStrings(classification.QuotaMarkers)
	classification.CapacityMarkers = DedupeStrings(classification.CapacityMarkers)
...
func DedupeCredentialKinds(in []CredentialKind) []CredentialKind {
	seen := make(map[CredentialKind]struct{}, len(in))
	out := make([]CredentialKind, 0, len(in))
	for _, kind := range in {
		if kind == "" {
			continue
		}
