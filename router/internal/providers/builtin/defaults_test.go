...
func TestDefaultRegistryCoversAllProviderDefinitions(t *testing.T) {
	...
	for _, definition := range auth.Definitions() {
		definitions = append(definitions, fromAuthDefinition(definition))
	}
	...
	for _, definition := range definitions {
		...
		if definition.Protocol != "" && string(metadata.Protocol) != string(definition.Protocol) {
			t.Fatalf("provider %q protocol = %q, want %q", id, metadata.Protocol, definition.Protocol)
		}
		if definition.CredentialKind != "" && string(metadata.CredentialKind) != string(definition.CredentialKind) {
