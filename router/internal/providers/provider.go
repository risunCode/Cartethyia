...
// ProviderCaps is the explicit capability record a Provider publishes.
// Provider-owned definitions populate the aggregate record; model entries may
// refine it without moving ownership into adapters or accounts.
type ProviderCaps struct {
	...
	Search bool
	// Batch reports whether the provider accepts asynchronous batch requests.
	// Batch is intentionally separate from streaming: a provider may support
	// either surface independently.
	Batch bool
	// Auth describes the account credential requirements for this provider.
	Auth AuthRequirements
	// Quota describes account/quota state required for selection.
	Quota QuotaRequirements
	// Classification contains provider-specific facts used to interpret a
	// bounded ClassifiedResponse without consulting an adapter.
...
// AuthRequirements describes the credential contract for a provider. Account
// lifecycle and refresh behavior remain owned by accounts; this record only
// tells selection which credential classes are valid or required.
...
// QuotaRequirements describes the quota state that selection may need from an
// account. It does not store quota values and does not perform quota I/O.
...
// ClassificationFacts records stable provider-specific classification facts.
// The response classifier remains the owner of classification behavior; these
// facts let router selection interpret its result without reaching into an
// adapter or account implementation.
type ClassificationFacts struct {
	AuthMarkers        []string
	EntitlementMarkers []string
	QuotaMarkers       []string
	CapacityMarkers    []string
	AuthScope          FailureScope
	EntitlementScope   FailureScope
	QuotaScope         FailureScope
	CapacityScope     FailureScope
}

// ProviderAuthRequirements is the descriptive alias used by catalog callers.
type ProviderAuthRequirements = AuthRequirements

// ProviderQuotaRequirements is the descriptive alias used by catalog callers.
type ProviderQuotaRequirements = QuotaRequirements

// ProviderClassification is the descriptive alias used by catalog callers.
type ProviderClassification = ClassificationFacts

// CloneProviderCaps returns an owned copy of a capability record.
func CloneProviderCaps(caps ProviderCaps) ProviderCaps {
	caps.Surfaces = append([]Surface(nil), caps.Surfaces...)
	caps.MediaGeneration = append([]string(nil), caps.MediaGeneration...)
	caps.Auth.CredentialKinds = append([]CredentialKind(nil), caps.Auth.CredentialKinds...)
	caps.Classification.AuthMarkers = append([]string(nil), caps.Classification.AuthMarkers...)
	caps.Classification.EntitlementMarkers = append([]string(nil), caps.Classification.EntitlementMarkers...)
	caps.Classification.QuotaMarkers = append([]string(nil), caps.Classification.QuotaMarkers...)
	caps.Classification.CapacityMarkers = append([]string(nil), caps.Classification.CapacityMarkers...)
...
// Supports is the value-oriented spelling of HasCapability used by router
// selection code.
func (caps ProviderCaps) Supports(surface Surface) bool {
	return HasCapability(caps, surface)
}
