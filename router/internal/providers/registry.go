...
type LoaderFactory struct {
	...
	Surfaces []Surface
	// Capabilities allows routing to inspect a lazy provider without
	// materializing its adapter.
...
func (r *Registry) RegisterLazy(factory LoaderFactory) error {
	...
	factory.Surfaces = append([]Surface(nil), factory.Surfaces...)
	factory.Capabilities = CloneProviderCaps(factory.Capabilities)
	if len(factory.Capabilities.Surfaces) == 0 && len(factory.Surfaces) > 0 {
		factory.Capabilities.Surfaces = append([]Surface(nil), factory.Surfaces...)
	}
	r.mu.Lock()
	defer r.mu.Unlock()
...
// Capabilities returns a provider's explicit capability record. A lazy
// provider is answered from its LoaderFactory and is not materialized when
// that factory declares capabilities.
func (r *Registry) Capabilities(providerID string) (ProviderCaps, error) {
	if r == nil {
		return ProviderCaps{}, ErrRegistryUnavailable
	}
	r.mu.RLock()
	if provider, ok := r.eager[providerID]; ok {
		caps := provider.Capabilities()
		r.mu.RUnlock()
		return CloneProviderCaps(caps), nil
	}
	if factory, ok := r.lazy[providerID]; ok {
		caps := CloneProviderCaps(factory.Capabilities)
		if len(caps.Surfaces) == 0 {
			caps.Surfaces = append([]Surface(nil), factory.Surfaces...)
		}
		r.mu.RUnlock()
		if hasCapabilityData(caps) {
			return caps, nil
		}
	} else {
		r.mu.RUnlock()
		return ProviderCaps{}, &UnknownProviderError{ProviderID: providerID}
	}
	provider, err := r.Get(providerID)
	if err != nil {
		return ProviderCaps{}, err
	}
	return CloneProviderCaps(provider.Capabilities()), nil
}
