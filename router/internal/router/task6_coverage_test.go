...
func (a *coverageQuotaAuthority) Reserve(_ context.Context, req ReservationRequest) (TokenReservation, error) {
	a.reserves.Add(1)
	if a.fail != nil {
		return nil, a.fail
	}
...
func (r *coverageQuotaReservation) Release(context.Context, ReleaseReason) error {
	r.releases.Add(1)
	return r.releaseErr
}
...
func (p *coveragePreparer) Prepare(_ context.Context, account Account, request contracts.Request) (*PreparedAttempt, error) {
	...
	return NewPreparedAttemptWithDecision(account, request, contracts.CompatibilityDecision{}, func() error {
		p.closes.Add(1)
		return nil
	})
}
...
func TestReserveAttemptUsesTokenBudgetAuthority(t *testing.T) {
	...
	ctx := WithAuthority(context.Background(), authority, Identity{
...
func TestReserveAttemptRejectsInvalidEstimate(t *testing.T) {
	router := &Router{defaultOutputCap: MaxTokenCount}
	authority := &coverageQuotaAuthority{}
	ctx := WithAuthority(context.Background(), authority, Identity{
		KeyID: "key-1", RequestID: "req-1", WindowUTC: time.Now().UTC().Truncate(time.Hour),
	})
	huge := contracts.Request{Body: bytes.Repeat([]byte("x"), 64)}
	if _, err := router.reserveAttempt(ctx, huge, 1); !errors.Is(err, ErrInvalid) {
...
func TestAttachPreparedAttemptTransfersCloser(t *testing.T) {
	...
	late := atomic.Bool{}
	lateAttempt, err := NewPreparedAttemptWithDecision(
...
func TestValidateContinuationRejectsUnknownBinding(t *testing.T) {
	store := NewContinuationStore(time.Hour)
	t.Cleanup(func() { _ = store.Close(context.Background()) })
	if err := store.Put(context.Background(), State{
...
func TestDiscardHedgeCandidateAndQuotaPersistence(t *testing.T) {
	...
	closed := false
	attempt, err := NewPreparedAttemptWithDecision(Account{ID: "a"}, contracts.Request{Protocol: contracts.SurfaceOpenAIChat, Model: "m"}, contracts.CompatibilityDecision{}, func() error {
		closed = true
		return nil
	})
	if err != nil {
