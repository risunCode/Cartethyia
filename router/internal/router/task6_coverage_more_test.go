...
func TestRecordContinuationAndFailureExhaustion(t *testing.T) {
	...
	if _, err := store.ResolveFor(context.Background(), "resp_new", Binding{Scope: "scope-a", Provider: "openai", Model: "model"}); err != nil {
...
func TestPrepareFailureObservationAndNewPreparedAttemptValidation(t *testing.T) {
	...
	if _, err := NewPreparedAttemptWithDecision(Account{}, contracts.Request{Protocol: contracts.SurfaceOpenAIChat, Model: "m"}, contracts.CompatibilityDecision{}, nil); err == nil {
		t.Fatal("empty account accepted")
	}
	if _, err := NewPreparedAttemptWithDecision(Account{ID: "a"}, contracts.Request{}, contracts.CompatibilityDecision{}, nil); err == nil {
