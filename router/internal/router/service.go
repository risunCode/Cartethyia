...
import (
	...
	runtimecache "github.com/cartethyia/daemon/internal/router/cache"
...
// implementation.
var defaultTokenSaver = NewOrchestrator()
...
type MetadataWriter interface {
	Enqueue(telemetry.Metadata) error
}
...
func (s *DispatchService) DispatchContext(ctx context.Context, req *contracts.Request) (result apicontracts.Stream, retErr error) {
	...
	defer func() {
		...
		if panicValue != nil {
			meta.Outcome = telemetry.OutcomeError
	...
	if err != nil {
		if s.Evidence != nil {
			...
			if errors.As(err, &capabilityErr) && capabilityErr != nil {
				s.Evidence.ObserveCapability(telemetry.CapabilityEvidence{Code: capabilityErr.Code, Operation: operationLabel(capabilityErr.Operation), Feature: string(capabilityErr.Feature)})
			}
		}
		return nil, err
	}
	...
	if req.Stream {
		...
		stream.AttachInFlight(s.InFlight, trackedInFlight)
		...
		inFlightID = ""
		deferMetadata = false
		decision, _ := stream.CompatibilityDecision()
		return &streamResponse{
			status: http.StatusOK, contentType: "text/event-stream",
			headers: http.Header{"Cache-Control": []string{"no-cache"}},
			stream:  stream, surface: req.Protocol, model: req.Model, codecs: s.Codecs, decision: decision,
		}, nil
	}
...
func (s *DispatchService) responseCacheGet(ctx context.Context, req contracts.Request, plan catalog.RoutePlan) (*contracts.Response, bool) {
	...
	if err != nil || len(entry.Value) == 0 {
		if s.Evidence != nil {
			s.Evidence.ObserveCache(telemetry.CacheEvidence{Kind: telemetry.CacheKindResponseL0, Layer: "l0", Operation: telemetry.CacheLookup, Outcome: "miss"})
		}
		return nil, false
	}
	...
	if err := validateProviderResponse(response); err != nil {
		if s.Evidence != nil {
			s.Evidence.ObserveCache(telemetry.CacheEvidence{Kind: telemetry.CacheKindResponseL0, Layer: "l0", Operation: telemetry.CacheReject, Outcome: "rejected"})
		}
		return nil, false
	}
	...
	if s.Evidence != nil {
		s.Evidence.ObserveCache(telemetry.CacheEvidence{Kind: telemetry.CacheKindResponseL0, Layer: "l0", Operation: telemetry.CacheHit, Outcome: "hit"})
	}
...
func (s *DispatchService) responseCacheSet(ctx context.Context, req contracts.Request, plan catalog.RoutePlan, response *contracts.Response) {
	...
	if s.Evidence != nil {
		op := telemetry.CacheWrite
		outcome := "stored"
		if err != nil {
			op, outcome = telemetry.CacheReject, "rejected"
		}
		s.Evidence.ObserveCache(telemetry.CacheEvidence{Kind: telemetry.CacheKindResponseL0, Layer: "l0", Operation: op, Outcome: outcome})
	}
}
func (s *DispatchService) validateRequest(ctx context.Context, req *contracts.Request) (*contracts.Request, error) {
	...
	if !json.Valid(copyReq.Body) {
		return nil, dispatchError(codeDispatchInvalidRequest, contracts.ErrorInvalidRequest, http.StatusBadRequest, "request body is malformed", nil)
	}
	...
	if copyReq.Protocol == contracts.ProtocolOpenAIChat || copyReq.Protocol == contracts.ProtocolOpenAIResponse || copyReq.Protocol == contracts.ProtocolAnthropic {
		// PASS forwards the original bytes. PATCH performs only bounded known
		// field edits (currently model suffix cleanup), without a canonical
		// decode/encode or ordinary map representation.
		preparation, prepareErr := PrepareSameSurfaceRequest(ctx, copyReq.Protocol, copyReq.Model, copyReq.Body)
		if prepareErr != nil {
			return nil, dispatchError(codeDispatchInvalidRequest, contracts.ErrorInvalidRequest, http.StatusBadRequest, "request preparation failed", prepareErr)
		}
		copyReq.Body = preparation.Body
		copyReq.Model = preparation.Model
		...
		if s.Evidence != nil {
			s.Evidence.ObserveCache(telemetry.CacheEvidence{Kind: telemetry.CacheKindTokenSaverL0, Layer: "l0", Operation: telemetry.CacheLookup, Outcome: "miss"})
		}
		if saverReq, changed := applyTokenSaver(ctx, prepared.Request); changed {
			...
			// the prepared body; the fast path deferred this work.
			...
			if s.Evidence != nil {
				s.Evidence.ObserveCache(telemetry.CacheEvidence{Kind: telemetry.CacheKindTokenSaverL0, Layer: "l0", Operation: telemetry.CacheWrite, Outcome: "stored"})
			}
...
// applyTokenSaver adapts the canonical request to the router optimizer's
...
func applyTokenSaver(ctx context.Context, req *transforms.NormalizedRequest) (*transforms.NormalizedRequest, bool) {
	...
	view := Request{Model: req.Model, Messages: make([]Message, len(req.Messages))}
	for mi, message := range req.Messages {
		view.Messages[mi] = Message{Role: string(message.Role), Content: make([]Block, len(message.Content))}
		for bi, block := range message.Content {
			kind := BlockOther
			if block.Type == transforms.BlockToolResult {
				kind = BlockToolResult
			}
			view.Messages[mi].Content[bi] = Block{
...
func (s *DispatchService) observeFailureExhaustion(failure *Failure) {
	...
	reason := telemetry.ExhaustionCandidate
	switch failure.Kind {
	case FailureRateLimit, FailureTransient, FailureServerError, FailureCapacity:
		reason = telemetry.ExhaustionNetwork
	case FailureAuthentication, FailureReauthenticationRequired:
		reason = telemetry.ExhaustionCredential
	case FailureQuota:
		reason = telemetry.ExhaustionQuota
	case FailureTranslation, FailureUnsupported:
		reason = telemetry.ExhaustionTranslation
	}
...
func (s *DispatchService) observeRoutePlan(req contracts.Request, plan catalog.RoutePlan) {
	...
	s.Evidence.ObserveOperation(telemetry.OperationEvidence{Operation: operationLabel(plan.Operation), CompactionVersion: version, Bridge: "none", Outcome: "planned"})
	for _, exclusion := range plan.Exclusions {
		s.Evidence.ObserveCapability(telemetry.CapabilityEvidence{Code: exclusion.Code, Operation: operationLabel(plan.Operation), Feature: string(exclusion.Feature)})
	}
}
...
func (s *DispatchService) enqueueMetadata(meta telemetry.Metadata) {
	if s.Metadata != nil {
...
func (s *DispatchService) finalizeStream(ctx context.Context, req contracts.Request, stream *Stream, meta *telemetry.Metadata, streamErr error) {
...
func parseUsage(body []byte) usage.Tokens {
	if tokens, ok := usage.ExtractProviderTokens("", "", body); ok {
		return tokens
	}
	var payload struct {
		Usage struct {
	...
	return usage.Tokens{
		Input: requestTokenPtr(input), Output: requestTokenPtr(output), Total: requestTokenPtr(total),
	}
}

func requestTokenPtr(value int64) *int64 {
	if value == 0 {
		return nil
	}
	return &value
...
func (s *DispatchService) recordContinuationID(ctx context.Context, req contracts.Request, responseID string) error {
	...
	if err := s.Continuations.Put(ctx, State{
...
func continuationBinding(req contracts.Request) (Binding, error) {
	if req.ContinuationScope == "" {
		return Binding{}, ContinuationErrUnauthorized
	}
	provider := defaultProviderForSurface(req.Protocol)
	if req.Model == "" {
		return Binding{}, &ContinuationError{
			Code: ContinuationCodeInvalid, Op: "binding", Message: "continuation model is required",
		}
	}
	return Binding{Scope: req.ContinuationScope, Provider: provider, Model: req.Model}, nil
}
...
func (r *streamResponse) Body() apicontracts.StreamReader {
	...
	defer r.mu.Unlock()
	if r.reader == nil {
		if r.decision.Mode == contracts.ModePass &&
			r.decision.SourceSurface == r.decision.TargetSurface &&
			r.decision.SourceSurface == r.surface {
			r.reader = NewRawStreamBridge(r.stream, r.surface, r.model)
		} else {
