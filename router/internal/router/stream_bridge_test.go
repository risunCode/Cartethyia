...
func TestRawStreamBridgePreservesFramesAndOneTerminal(t *testing.T) {
	events := make(chan StreamEvent, 2)
	frames := []byte("event: response.output_text.delta\r\ndata: {\"delta\":\"a\"}\r\n\r\n")
	terminal := []byte("event: response.completed\ndata: {\"status\":\"completed\",\"unknown\":{\"keep\":true}}\n\n")
	events <- StreamEvent{Kind: EventTextDelta, RawFrame: frames}
	events <- StreamEvent{Kind: EventMessageStop, Reason: "completed", RawFrame: terminal}
	close(events)
	var closes atomic.Int32
	stream := NewStream(events, func() { closes.Add(1) }, 0, 0)
	body, err := readBridge(t, NewRawStreamBridge(stream, contracts.SurfaceOpenAIResponses, "gpt"))
	if !errors.Is(err, io.EOF) {
		t.Fatalf("raw bridge read error = %v, want EOF", err)
	}
	if !bytes.Equal([]byte(body), append(frames, terminal...)) {
		t.Fatalf("raw bridge changed frame bytes: got %q, want %q", body, append(frames, terminal...))
	}
	if got := bytes.Count([]byte(body), []byte("response.completed")); got != 1 {
		t.Fatalf("terminal frame count = %d, want one", got)
	}
	if got := closes.Load(); got != 1 {
		t.Fatalf("source close count = %d, want one", got)
	}
}
