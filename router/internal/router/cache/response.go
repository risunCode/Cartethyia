...
func (c *ResponseCache) key(spec ResponseSpec) (Key, error) {
	...
	if strings.TrimSpace(spec.TenantID) == "" || strings.TrimSpace(spec.SourceSurface) == "" || strings.TrimSpace(spec.TargetSurface) == "" || strings.TrimSpace(spec.Provider) == "" || strings.TrimSpace(spec.Model) == "" || strings.TrimSpace(spec.RequestBodyDigest) == "" || spec.Generation.IsZero() ||
		len(spec.TenantID) > maxSharedTenantBytes || len(spec.RequestBodyDigest) > maxSharedIdentityBytes ||
		len(spec.Provider) > maxSharedNamespaceBytes || len(spec.Model) > maxSharedNamespaceBytes ||
		len(spec.SourceSurface) > maxSharedNamespaceBytes || len(spec.TargetSurface) > maxSharedNamespaceBytes ||
		strings.ContainsRune(spec.TenantID, '\x00') || strings.ContainsRune(spec.RequestBodyDigest, '\x00') ||
		strings.ContainsRune(spec.Provider, '\x00') || strings.ContainsRune(spec.Model, '\x00') {
	...
	return NewKey(spec.Model, spec.SourceSurface+"->"+spec.TargetSurface,
		[]CapabilityRequirement{"response", CapabilityRequirement(hex.EncodeToString(digestSum[:])), CapabilityRequirement(hex.EncodeToString(tenantSum[:]))},
