...
// NewRawStreamBridge forwards provider SSE frames verbatim. The source still
// owns cancellation, accounting, and terminal finalization; this bridge only
// performs a bounded side tap for the terminal error/usage state.
func NewRawStreamBridge(source *Stream, surface contracts.Surface, model string) *StreamBridge {
...
func (b *StreamBridge) ReadContext(ctx context.Context, p []byte) (int, error) {
	...
	for len(b.pending) == 0 && !b.finished {
		...
		var encodeErr error
			if b.raw {
				if len(ev.RawFrame) > 0 {
					// RawFrame already contains the complete SSE event,
					// including its original delimiters. Never reconstruct or
					// canonicalize it.
					frames = ev.RawFrame
				}
