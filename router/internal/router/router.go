...
type RouterConfig struct {
	...
	MaxHedges         int
	Observer          telemetry.AttemptObserver
...
func NewRouter(cfg RouterConfig) (*Router, error) {
	...
	if defaultOutputCap <= 0 || defaultOutputCap > MaxTokenCount {
...
func (r *Router) Route(ctx context.Context, transport Transport, req contracts.Request, plan catalog.RoutePlan) (*contracts.Response, *Failure, error) {
	...
	coordinator := newAttemptCoordinator(r, req, plan)
	state := coordinator.state
	hedges := 0
	defer func() { r.observeRequestAttempts(state.attempts) }()
	defer coordinator.closeIfOpen()

	for {
		progress := coordinator.next(ctx)
		if progress.failure != nil {
			return nil, progress.failure, nil
		}
		if progress.err != nil {
			return nil, nil, progress.err
		}
		if progress.done {
			failure, err := coordinator.terminal()
			if err != nil {
		...
		acct, member := candidate.account, candidate.member
		prepared, preparedReq, lease := candidate.prepared, candidate.req, candidate.lease
		var resp *contracts.Response
		var callErr error
		startedAt, endedAt := r.currentTime().UTC(), r.currentTime().UTC()
		var network attemptNetworkSnapshot
		if hedges < r.maxHedges && candidate.attempt == 1 && r.hedgeEligible(req, plan) {
			outcome, hedged := r.executeHedge(ctx, transport, req, candidate, state, state.memberRequests, plan)
			if hedged {
				hedges++
			}
			candidate = outcome.candidate
			acct, member, state.memberIndex, preparedReq = candidate.account, candidate.member, candidate.memberIndex, candidate.req
		...
		if callErr == nil {
			...
			evidence.Result = telemetry.AttemptSucceeded
			...
			if candidate.reservation != nil {
				if reconcileErr := candidate.reservation.Reconcile(ctx, attemptUsage); reconcileErr != nil {
					r.quotaPersistenceFailures.Add(1)
				}
			}
			coordinator.complete(resp)
			return resp, nil, nil
		}
		failure := coordinator.classifyAttempt(ctx, candidate, callErr, true)
		if repair, accepted := coordinator.repair(transport, candidate, callErr); repair.RuleID != "" && accepted {
			evidence.Result = telemetry.AttemptFailed
		...
		state.repairRule = ""
		failure, decision := coordinator.classify(ctx, candidate, callErr, true)
		evidence.Result = telemetry.AttemptFailed
		evidence.Code, evidence.Scope, evidence.Phase = failure.Code, string(failure.Scope), string(failure.Phase)
		...
		if retry, retryDecision := coordinator.retry(ctx, candidate, decision); retry {
			_ = retryDecision
			continue
		} else if retryDecision.Action == RetryStop || !retryDecision.AlternateAccount {
...
func (r *Router) RouteStream(ctx context.Context, transport StreamTransport, req contracts.Request, plan catalog.RoutePlan) (*Stream, string, *Failure, error) {
	...
	for {
		...
		if prepared != nil {
			attemptCtx = WithCompatibilityDecision(attemptCtx, prepared.Decision)
		}
		...
		if callErr == nil {
			...
			if candidate.reservation != nil {
				stream.AttachTokenReservation(ctx, candidate.reservation)
			}
			...
			if preflightErr != nil {
				...
				if !stream.Committed() {
					if repair, accepted := coordinator.repair(transport, candidate, preflightErr); repair.RuleID != "" && accepted {
				...
				evidence.Failover = state.failover(acct.ID, state.memberIndex)
				r.observeAttempt(evidence)
				stream.AttachFinalizationEvidence(r.observer, telemetry.StreamFinalizationEvidence{
		...
		failure, decision := coordinator.classify(ctx, candidate, callErr, stream == nil)
		state.repairRule = ""
		evidence.Result = telemetry.AttemptFailed
		evidence.Code, evidence.Scope, evidence.Phase = failure.Code, string(failure.Scope), string(failure.Phase)
		evidence.RetryAction = string(decision.Action)
		...
		if stream != nil && (stream.Committed() || stream.ClientVisible()) {
			return nil, "", failure, nil
		}
		if retry, retryDecision := coordinator.retry(ctx, candidate, decision); retry {
			_ = retryDecision
...
func (r *Router) discardHedgeCandidate(ctx context.Context, candidate *hedgeCandidate) {
	...
	if candidate.reservation != nil {
		if err := candidate.reservation.Release(ctx, ReleaseUnaccepted); err != nil {
			r.quotaPersistenceFailures.Add(1)
		}
	}
...
func (r *Router) reserveAttempt(ctx context.Context, req contracts.Request, attempt int) (TokenReservation, error) {
	authority, identity, ok := AuthorityFromContext(ctx)
	if !ok {
		return nil, nil
	}
	...
	return authority.Reserve(ctx, ReservationRequest{
		KeyID: identity.KeyID, RequestID: identity.RequestID, Attempt: attempt,
		WindowUTC: identity.WindowUTC, Estimate: estimate,
	})
}

func (r *Router) releaseProvenUnaccepted(ctx context.Context, reservation TokenReservation, failure *Failure) {
	if reservation == nil || failure == nil || failure.Phase != FailurePhasePreDispatch {
		return
	}
	if err := reservation.Release(ctx, ReleaseUnaccepted); err != nil {
		r.quotaPersistenceFailures.Add(1)
	}
}
...
func estimateAttemptTokens(req contracts.Request, defaultOutputCap int64) (int64, error) {
	...
	if inputEstimate > MaxTokenCount-outputCap {
		return 0, ErrInvalid
	}
	estimate := inputEstimate + outputCap
	if estimate < 1 || estimate > MaxTokenCount {
		return 0, ErrInvalid
	}
	return estimate, nil
}
...
func (r *Router) attemptEvidence(req contracts.Request, plan catalog.RoutePlan, memberIndex int, member catalog.RouteMember, acct Account, network attemptNetworkSnapshot, attempt int, repairRule string, startedAt, endedAt time.Time) telemetry.AttemptEvidence {
	...
	if req.Stream {
		surface = string(telemetry.SurfaceStream)
	}
	return telemetry.AttemptEvidence{
...
func (r *Router) observeAttempt(evidence telemetry.AttemptEvidence) {
...
func (r *Router) observeExclusions(req contracts.Request, plan catalog.RoutePlan, memberIndex int, member catalog.RouteMember, availability Availability) {
	...
	for _, exclusion := range availability.Exclusions {
		...
		evidence := telemetry.CandidateExclusionEvidence{
			RequestID: headerValue(req.Headers, "X-Request-ID"), CatalogGeneration: plan.Generation,
...
func (r *Router) observeRepair(req contracts.Request, plan catalog.RoutePlan, memberIndex, attempt int, repair RepairEvidence, applied bool) {
	...
	evidence := telemetry.RepairEvidence{
		RequestID: headerValue(req.Headers, "X-Request-ID"), CatalogGeneration: plan.Generation,
...
func attemptTokenUsage(tokens usage.Tokens) telemetry.TokenUsage {
	var result telemetry.TokenUsage
	set := func(mask telemetry.UsageMask, value *int64, target *int64) {
		if value != nil {
			result.Known |= mask
			*target = *value
		}
	}
	set(telemetry.UsageInput, tokens.Input, &result.Input)
	set(telemetry.UsageOutput, tokens.Output, &result.Output)
	set(telemetry.UsageCachedRead, tokens.CachedRead, &result.CachedRead)
	set(telemetry.UsageCachedWrite, tokens.CachedWrite, &result.CachedWrite)
	set(telemetry.UsageReasoning, tokens.Reasoning, &result.Reasoning)
	set(telemetry.UsageTotal, tokens.Total, &result.Total)
	return result
}
