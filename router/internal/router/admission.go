...
const (
	...
	CodeDuplicateLayer     Code = "duplicate_layer"
	CodeInvalidLimit       Code = "invalid_limit"
	CodeClosed             Code = "closed"
	CodeLimit              Code = "limit_reached"
	CodeCanceled           Code = "canceled"
	CodeWaiterLimit        Code = "waiter_limit"
	CodeInvalidReservation Code = "invalid_reservation"
	CodeReleased           Code = "released"
	CodeReconciled         Code = "already_reconciled"
)
...
// AdmissionCodeOf returns the package-owned code from err, or "" for unrelated errors.
func AdmissionCodeOf(err error) string {
...
var (
	...
	AdmissionErrLimit = &Error{Code: CodeLimit}
...
func NewAdmissionLimiter(layers ...Layer) (*Limiter, error) {
...
func (l *Limiter) release(usage map[string]int, keys map[string]string) {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.releaseUsageLocked(usage, keys)
	l.active.Add(-1)
}

func (l *Limiter) releaseUsage(usage map[string]int, keys map[string]string) {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.releaseUsageLocked(usage, keys)
}

func (l *Limiter) releaseUsageLocked(usage map[string]int, keys map[string]string) {
...
func (s *Reservation) Reconcile(actualTokens int) error {
	...
	if delta > 0 {
		...
		s.owner.releaseUsage(scaledTokenUsage(s.usage, -delta, s.owner.layers), s.keys)
