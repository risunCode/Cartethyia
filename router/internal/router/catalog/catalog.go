...
// SourceSnapshot is the bounded, generation-stamped source data captured
// before provider resolution. It is deliberately independent of provider
// registry state so callers can load once and resolve against a stable
// generation without re-reading the source.
...
func (s SourceSnapshot) Clone() SourceSnapshot {
	out := SourceSnapshot{
		...
		Combinations: make([]Combination, len(s.Combinations)),
		Generation:   s.Generation,
	}
	for i, combination := range s.Combinations {
		out.Combinations[i] = combination
		out.Combinations[i].Members = append([]string(nil), combination.Members...)
	}
	return out
}
...
// Clone returns a defensive copy of the plan. Plans are compiled from one
// catalog generation and may be shared by concurrent attempts; callers that
// need to modify attempt-local metadata must clone first.
func (p RoutePlan) Clone() RoutePlan {
	out := p
	out.Requirements.Hard = append([]FeatureRequirement(nil), p.Requirements.Hard...)
	out.Requirements.Soft = append([]FeatureRequirement(nil), p.Requirements.Soft...)
	out.Requirements.ToolKinds = append([]transforms.ToolKind(nil), p.Requirements.ToolKinds...)
	out.Requirements.ReferenceKinds = append([]string(nil), p.Requirements.ReferenceKinds...)
	out.Members = append([]RouteMember(nil), p.Members...)
	out.Exclusions = make([]RouteExclusion, len(p.Exclusions))
	for i, exclusion := range p.Exclusions {
		out.Exclusions[i] = exclusion
		out.Exclusions[i].Supported = append([]string(nil), exclusion.Supported...)
	}
	if p.CanonicalOperation.Compaction != nil {
		if cloned, err := transforms.NewCompactionRequest(*p.CanonicalOperation.Compaction); err == nil {
			out.CanonicalOperation.Compaction = cloned
		} else {
			// Plans are validated before publication, so this is only a
			// defensive fallback for a plan assembled manually by a caller.
			request := *p.CanonicalOperation.Compaction
			request.Input = append([]transforms.CompactionItem(nil), request.Input...)
			request.Tools = append([]transforms.Tool(nil), request.Tools...)
			request.Include = append([]string(nil), request.Include...)
			if request.Reasoning != nil {
				reasoning := *request.Reasoning
				request.Reasoning = &reasoning
			}
			out.CanonicalOperation.Compaction = &request
		}
	}
	return out
}
...
func (s *Snapshot) Resolve(modelID string) (Model, error) {
	...
	if qualified, ok := s.Unqualified[modelID]; ok {
		...
		return cloneModel(s.Models[qualified]), nil
	}
...
func (b *Builder) Build(ctx context.Context, source Source) (*Snapshot, error) {
	input, err := b.Load(ctx, source)
	if err != nil {
		return nil, err
	}
	return b.Resolve(input)
}

// Load captures source aliases, combinations, and the catalog generation
// without consulting provider models. The returned value owns defensive
// copies, allowing the source implementation to reuse its buffers.
func (b *Builder) Load(ctx context.Context, source Source) (SourceSnapshot, error) {
	if source == nil {
		return SourceSnapshot{}, errors.New("catalog: source is required")
	}
	aliases, combos, generation, err := source.Load(ctx)
	if err != nil {
		return SourceSnapshot{}, fmt.Errorf("catalog source: %w", err)
	}
	out := SourceSnapshot{
		Aliases: append([]Alias(nil), aliases...),
		Combinations: make([]Combination, len(combos)),
		Generation: generation,
	}
	for i, combo := range combos {
		out.Combinations[i] = combo
		out.Combinations[i].Members = append([]string(nil), combo.Members...)
	}
	return out, nil
}

// Resolve compiles one immutable provider-aware snapshot from source data.
// Resolution never reloads the source and therefore cannot silently mix
// aliases from one generation with provider capabilities from another.
func (b *Builder) Resolve(input SourceSnapshot) (*Snapshot, error) {
	if b == nil || b.registry == nil {
		return nil, errors.New("catalog: builder is unavailable")
	}
	b.mu.Lock()
	defer b.mu.Unlock()
	input = input.Clone()
...
func cloneModel(model Model) Model {
	model.Surfaces = append([]providers.Surface(nil), model.Surfaces...)
	model.Capabilities.Surfaces = append([]providers.Surface(nil), model.Capabilities.Surfaces...)
	model.Capabilities.MediaGeneration = append([]string(nil), model.Capabilities.MediaGeneration...)
	model.Capabilities.Compatibility = model.Capabilities.Compatibility.Clone()
	return model
}

func cloneProviderCaps(caps providers.ProviderCaps) providers.ProviderCaps {
	return providers.CloneProviderCaps(caps)
}
