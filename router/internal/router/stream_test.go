...
func (s *streamQuotaSpy) Release(context.Context, ReleaseReason) error {
	s.releases++
	return nil
}
...
func TestStreamReconcilesDurableReservationOnceForEveryOutcome(t *testing.T) {
	for _, test := range []struct {
		name    string
		outcome telemetry.StreamOutcome
		trigger func(*Stream)
	}{
		{name: "clean", outcome: telemetry.StreamClean, trigger: func(stream *Stream) {
			stream.ch <- StreamEvent{Kind: EventMessageStop, Reason: "completed"}
			_, _ = stream.Next(context.Background())
		}},
		{name: "failed", outcome: telemetry.StreamFailed, trigger: func(stream *Stream) {
			stream.ch <- StreamEvent{Kind: EventMessageStop, Reason: "error", Err: ErrStreamUpstream}
			_, _ = stream.Next(context.Background())
		}},
		{name: "canceled", outcome: telemetry.StreamCanceled, trigger: func(stream *Stream) { stream.Abort(context.Canceled) }},
		{name: "stalled", outcome: telemetry.StreamStalled, trigger: func(stream *Stream) {
			stream.Abort(streamError(StreamCodeIdleTimeout, "stream idle timeout", ErrStreamStall))
		}},
		{name: "truncated", outcome: telemetry.StreamTruncated, trigger: func(stream *Stream) {
			close(stream.ch)
			_, _ = stream.Next(context.Background())
		}},
		{name: "downstream write", outcome: telemetry.StreamDownstreamWrite, trigger: func(stream *Stream) {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			recorder := telemetry.NewRecorder(context.Background(), sink, telemetry.WithCapacity(4))
			...
			stream.AttachFinalizationEvidence(registry, telemetry.StreamFinalizationEvidence{
			...
			events := sink.Events()
			if len(events) != 1 || events[0].Stage != telemetry.StageStreamFinalization {
