...
const (
	...
	EventRawFrame         StreamEventKind = "raw_frame"
...
type StreamEvent struct {
	...
	CallName string
	Reason   string
	// RawFrame carries bounded, provider-framed SSE bytes when the transport
	// can preserve them. A raw surface bridge forwards these bytes verbatim.
	RawFrame []byte
...
type Stream struct {
	...
	evidenceObserver telemetry.AttemptObserver
	evidence         telemetry.StreamFinalizationEvidence
	usage            StreamUsage
	usageSeen        bool
	responseID       string
...
func (s *Stream) AttachAdmissionLease(lease *Lease) {
	if s == nil {
		if lease != nil {
			lease.Release()
		}
...
func (s *Stream) AttachTokenReservation(ctx context.Context, reservation TokenReservation) {
	if reservation == nil {
		return
	}
	if s == nil {
		_ = reservation.Release(context.Background(), ReleaseUnaccepted)
		return
	}
	if ctx == nil {
...
func (s *Stream) AttachFinalizer(finalize func(error, error)) {
	...
	if s.closed.Load() {
		err := s.err
		s.mu.Unlock()
		s.finalizerOnce.Do(func() { finalize(err, nil) })
		return
	}
	s.finalize = finalize
	s.mu.Unlock()
...
// AttachInFlight transfers removal of one live-dispatch projection to the
// stream lifecycle. The callback is run by the same exactly-once finalizer as
// leases, including cancellation and downstream aborts.
func (s *Stream) AttachInFlight(registry *InFlightRegistry, id string) {
	if registry == nil || id == "" {
		return
	}
	if s == nil {
	...
	release := func() { registry.Release(id) }
	s.mu.Lock()
	if s.closed.Load() {
		s.mu.Unlock()
		release()
...
func (s *Stream) AttachFinalizationEvidence(observer telemetry.AttemptObserver, evidence telemetry.StreamFinalizationEvidence) {
	if observer == nil {
		return
	}
...
func (s *Stream) processEvent(ev StreamEvent) (StreamEvent, error) {
	...
	if ev.IsTerminal() {
		...
		if s.terminalSeen {
			s.mu.Unlock()
			err := streamError(StreamCodeMalformedEvent, "stream emitted more than one terminal event", ErrStreamMalformed)
			s.markAborted(err)
			return StreamEvent{}, err
		}
		s.terminalSeen = true
...
func streamEventSize(ev StreamEvent) int {
	return 32 + len(ev.Text) + len(ev.CallID) + len(ev.CallName) + len(ev.Reason) + len(ev.Payload) + len(ev.RawFrame)
}
...
// MarkClientVisible records that at least one downstream frame has been
// returned to the caller. It is intentionally separate from Committed, which
// is the router's preflight semantic commit gate.
func (s *Stream) MarkClientVisible() {
	if s != nil {
		s.clientVisible.Store(true)
	}
}

// ClientVisible reports whether downstream framing has returned bytes to the
// caller.
func (s *Stream) ClientVisible() bool {
...
func knownStreamEventKind(kind StreamEventKind) bool {
	...
	case EventMessageStart, EventThinkingDelta, EventTextDelta,
		...
		EventServerToolResult, EventCompactionItem, EventUsage, EventRawFrame, EventMessageStop:
		return true
	default:
		return false
	}
...
func (s *Stream) finish(err error) {
	s.finishOnce.Do(func() {
		...
		finalize := s.finalize
		inFlightRelease := s.inFlightRelease
		...
		if finalize != nil {
			s.finalizerOnce.Do(func() { finalize(err, reconcileErr) })
		}
...
func finalizationOutcome(err error) telemetry.StreamOutcome {
	if err == nil {
		return telemetry.StreamClean
	}
	switch StreamCodeOf(err) {
	case StreamCodeClientDisconnect:
		return telemetry.StreamCanceled
	case StreamCodeIdleTimeout:
		return telemetry.StreamStalled
	case StreamCodeUpstreamTruncated:
		return telemetry.StreamTruncated
	case StreamCodeWriteFailure:
		return telemetry.StreamDownstreamWrite
	}
...
func (s *Stream) AttachPreparedAttempt(attempt *PreparedAttempt) {
	...
	s.decision = attempt.Decision
	s.decisionSet = attempt.Decision.Mode != contracts.ModePass ||
		attempt.Decision.SourceSurface.IsValid() || attempt.Decision.TargetSurface.IsValid()
	s.mu.Unlock()
}

// CompatibilityDecision returns the immutable decision selected for this
// stream attempt, when preparation supplied one.
func (s *Stream) CompatibilityDecision() (contracts.CompatibilityDecision, bool) {
	if s == nil {
		return contracts.CompatibilityDecision{}, false
	}
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.decision, s.decisionSet
}
