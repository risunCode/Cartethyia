...
type metadataCollector struct{ values []telemetry.Metadata }

func (c *metadataCollector) Enqueue(value telemetry.Metadata) error {
...
func (w *failingMetadataWriter) Enqueue(telemetry.Metadata) error {
	w.calls++
	return w.err
}
...
func newLifecycleService(t *testing.T, transport Transport, maxAttempts int) (*DispatchService, *Limiter) {
	t.Helper()
	...
	limiter, err := NewAdmissionLimiter(Layer{Name: "global", Limit: 1})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDispatchEnqueuesMetadataExactlyOnce(t *testing.T) {
	...
	if value.Outcome != telemetry.OutcomeSuccess || value.MessageCount != 1 || value.RequestID != "lifecycle-1" {
