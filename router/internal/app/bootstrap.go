...
import (
	...
	routerbatch "github.com/cartethyia/daemon/internal/router/batch"
	...
	"github.com/cartethyia/daemon/internal/router/cache"
	"github.com/cartethyia/daemon/internal/gateway"
	api "github.com/cartethyia/daemon/internal/gateway/api"
	apicontracts "github.com/cartethyia/daemon/internal/gateway/contracts"
	servermiddleware "github.com/cartethyia/daemon/internal/gateway/middleware"
	"net/http"
	"os"
	"strconv"
	"strings"
...
type shareInFlightSource struct{ limiter *proxy.Limiter }
...
// adminInFlightStats adapts the router limiter snapshot to the console
// InFlightStatsSource contract without widening the router package API.
type adminInFlightStats struct{ limiter *proxy.Limiter }
...
func (s adminInFlightDetail) InFlightRows() []consolecontracts.InFlightRow {
	if s.registry == nil {
		return nil
	}
	...
	now := time.Now()
	rows := make([]consolecontracts.InFlightRow, 0, len(records))
	for _, record := range records {
		...
		rows = append(rows, consolecontracts.InFlightRow{
			ID:        record.ID,
			Model:     record.Model,
...
func responseCacheTenant(ctx context.Context, _ contracts.Request) string {
	if _, identity, ok := proxy.AuthorityFromContext(ctx); ok && identity.KeyID != "" {
...
type BootstrapDependencies struct {
	...
	Credentials   egress.CredentialResolver
	...
	ProxySelector      egress.ProxySelector
	ProxyFailure       egress.ProxyFailureRecorder
	ProxySuccess       egress.ProxySuccessRecorder
	...
	// Batch is the optional durable batch lifecycle service exposed through
	// the public gateway and operator console.
	Batch routerbatch.Service
	...
	MetadataWriter *telemetry.AsyncMetadataWriter
	...
	TokenBudget proxy.TokenBudgetAuthority
}
...
func defaultBootstrapDependencies(configs ...Config) (BootstrapDependencies, error) {
	...
	driverRegistry, err := accounts.NewRegistry(nil)
	if err != nil {
	...
	deps := BootstrapDependencies{
		...
		Observability:  telemetry.NewRegistry().WithLogger(telemetry.NewLogger(nil, telemetry.LevelInfo)),
	}
	...
	if strings.EqualFold(cfg.Environment, "production") || strings.TrimSpace(cfg.DatabaseURL) != "" {
		...
		proxyCoordinator := egress.NewDurableProxyCoordinator(database.Proxies, deps.Observability)
		...
		deps.MetadataWriter = telemetry.NewAsyncMetadataWriter(
			context.Background(),
...
func buildHandlerWithArtworkAndDependencies(cfg Config, deps BootstrapDependencies, artwork string) (http.Handler, error) {
	...
	var console *consoleservices.ConsoleEventSink
	if deps.Database != nil && deps.Database.Telemetry != nil {
		...
		console = consoleservices.NewConsoleEventSink(context.Background(), deps.Database.Telemetry)
	}
	if metrics.Recorder() == nil {
		sink := telemetry.EventSink(telemetry.LogSink{Logger: metrics.Logger()})
		if console != nil {
			sink = consoleservices.FanOutEventSink(telemetry.LogSink{Logger: metrics.Logger()}, console)
		}
		metrics.WithRecorder(telemetry.NewRecorder(context.Background(), sink, telemetry.WithCapacity(telemetry.MaxConcurrentEvents)))
	}
	...
	httpTransport := &egress.HTTPTransport{
	...
	streamHTTPTransport := &egress.HTTPTransport{
		Registry:          deps.Registry,
		BaseURLs:          baseURLs,
		ResolveCredential: credentialResolver,
		ProxySelector:     deps.ProxySelector,
	...
	dispatch := &proxy.DispatchService{
		...
		Continuations:   proxy.NewContinuationStore(cfg.UsageRetention),
	...
	limiter, err := proxy.NewAdmissionLimiter(
		proxy.Layer{Name: "global", Limit: cfg.MaxConcurrent},
		proxy.Layer{Name: "stream", Limit: cfg.MaxConcurrentStream},
	)
	if err != nil {
	...
	var share *consoleapi.ShareOptions
	if deps.Database != nil && deps.Database.AdminAPIKeys != nil {
		share = &consoleapi.ShareOptions{
			APIKeys:  deps.Database.AdminAPIKeys,
			Usage:    deps.Database.Telemetry,
			InFlight: shareInFlightSource{limiter: limiter},
		}
	}
	if deps.Admin == nil && (deps.Database != nil || (deps.DriverRegistry != nil && deps.Accounts != nil && deps.Secrets != nil && deps.Records != nil)) {
		var proxies dbrepositories.ProxyRepository
		var apiKeys dbrepositories.APIKeyRepository
		var settings *dbrepositories.BunSettingsRepository
		var telemetryStore consoleservices.TelemetryStore
		if deps.Database != nil {
			proxies = deps.Database.Proxies
			apiKeys = deps.Database.AdminAPIKeys
			settings = deps.Database.Settings
			telemetryStore = deps.Database.Telemetry
		}
		services, servicesErr := consoleservices.BuildServices(consoleservices.BuildInput{
			Accounts:        deps.Accounts,
			Records:         deps.Records,
			Secrets:         deps.Secrets,
			Refresher:       refresher,
			...
			Console:         console,
			InFlightStats:   adminInFlightStats{limiter: limiter},
			InFlightDetail:  adminInFlightDetail{registry: inFlightRegistry},
		...
		if servicesErr != nil {
			return nil, fmt.Errorf("runtime: console composition: %w", servicesErr)
		}
		services.Batch = deps.Batch
		deps.Admin = consoleapi.NewRegistrar(services, share)
	} else if deps.Admin != nil && share != nil {
		owner := deps.Admin
		deps.Admin = registrarFunc(func(mux *http.ServeMux) {
			owner.Register(mux)
			consoleapi.RegisterShare(mux, share)
		})
	}
	base, err := gateway.NewRouterWith(gateway.Options{
		Registry:      metrics,
		HealthArtwork: artwork,
		DashboardDir:  cfg.DashboardDir,
		V1: registrarFunc(func(mux *http.ServeMux) {
			api.RegisterV1(mux, api.Deps{Proxy: dispatch, Catalog: publicCatalog, Evidence: metrics, Batch: deps.Batch})
		}),
		V1Auth:  servermiddleware.PublicV1Auth(deps.PublicAPIKeys, deps.TokenBudget, strings.EqualFold(cfg.Environment, "production")),
		Console: deps.Admin,
	})
	if err != nil {
		return nil, fmt.Errorf("runtime: gateway router: %w", err)
	}
	return base, nil
}
...
func composeCredentialResolver(deps BootstrapDependencies) (egress.CredentialResolver, error) {
	resolver, _, err := composeCredentialResolverWithInvalidator(deps)
	return resolver, err
}

func composeCredentialResolverWithInvalidator(deps BootstrapDependencies) (egress.CredentialResolver, func(string), error) {
