...
func batchJobs() Migration {
	return Migration{Version: 32, Name: "batch_jobs", Statements: []string{
  ...
  updated_at TIMESTAMPTZ NOT NULL
		...
		`CREATE INDEX IF NOT EXISTS idx_batch_jobs_state_created ON batch_jobs (state, created_at, id);`,
		`CREATE INDEX IF NOT EXISTS idx_batch_jobs_expiry ON batch_jobs (expires_at) WHERE state IN ('queued','running');`,
  ...
  UNIQUE (job_id, request_id)
		...
		`CREATE INDEX IF NOT EXISTS idx_batch_items_job_position ON batch_items (job_id, position);`,
		`CREATE INDEX IF NOT EXISTS idx_batch_items_state ON batch_items (state, updated_at, id);`,
	}}
}
