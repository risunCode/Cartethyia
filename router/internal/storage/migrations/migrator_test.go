...
// TestApplyRollsBackAfterEarlierStatement verifies that the transaction
// boundary covers every statement in a migration, not only the statement that
// failed. A prior successful DDL statement must not make the migration appear
// applied after a later statement fails.
func TestApplyRollsBackAfterEarlierStatement(t *testing.T) {
	drv := newFakeDriver()
	mig := NewSQLMigrator(drv)
	mig.migrations[0] = Migration{
		Version: 1,
		Name:    "partially_broken",
		Statements: []string{
			"CREATE TABLE partial (id INT);",
			"FAIL ALTER TABLE partial ADD COLUMN value TEXT;",
		},
	}
	drv.failNext(errors.New("constraint violation"))

	res, err := mig.Apply(context.Background())
	if err == nil {
		t.Fatal("Apply returned nil error, want a failure")
	}
	if !strings.Contains(err.Error(), "partially_broken") {
		t.Fatalf("error %q does not name the failing migration", err)
	}
	if len(res.Applied) != 0 || res.Final != 0 {
		t.Fatalf("result after rollback = applied %d final %d, want empty/0", len(res.Applied), res.Final)
	}
	if got := drv.rollbackCount(); got != 1 {
		t.Fatalf("rolled back transactions = %d, want 1", got)
	}
	if got := drv.commitCount(); got != 1 {
		t.Fatalf("committed transactions = %d, want 1 metadata transaction only", got)
	}
	if got := drv.appliedVersions(); len(got) != 0 {
		t.Fatalf("recorded metadata versions = %v, want none after rollback", got)
	}
...
// TestApplyRejectsDescendingVersions verifies that validation rejects a
// registry that is ordered neither increasingly nor contiguously.
func TestApplyRejectsDescendingVersions(t *testing.T) {
	drv := newFakeDriver()
	mig := NewSQLMigrator(drv)
	mig.migrations = []Migration{
		{Version: 1, Name: "a", Statements: []string{"CREATE TABLE a (id INT);"}},
		{Version: 3, Name: "c", Statements: []string{"CREATE TABLE c (id INT);"}},
		{Version: 2, Name: "b", Statements: []string{"CREATE TABLE b (id INT);"}},
	}
	...
	if got := drv.statementCount(); got != 0 {
		t.Fatalf("executed %d statements on ordering failure, want 0", got)
	}
}
...
// TestNewSQLMigratorUsesFinalMigrationRegistry verifies that the migrator
// cloned by the storage runtime is exactly the final storage registry. This
// keeps bootstrap from silently carrying a stale pre-move migration list.
func TestNewSQLMigratorUsesFinalMigrationRegistry(t *testing.T) {
	want := All()
	mig := NewSQLMigrator(newFakeDriver())
	if len(mig.migrations) != len(want) {
		t.Fatalf("migrator registry length = %d, want %d", len(mig.migrations), len(want))
	}
	for i, got := range mig.migrations {
		expected := want[i]
		if got.Version != expected.Version || got.Name != expected.Name {
			t.Fatalf("migrator registry[%d] = %d (%s), want %d (%s)", i, got.Version, got.Name, expected.Version, expected.Name)
		}
		if len(got.Statements) != len(expected.Statements) {
			t.Fatalf("migrator registry[%d] statement count = %d, want %d", i, len(got.Statements), len(expected.Statements))
		}
		for j, statement := range got.Statements {
			if statement != expected.Statements[j] {
				t.Fatalf("migrator registry[%d].Statements[%d] differs from All()", i, j)
			}
		}
	}
}

// TestStorageMigrationsDoNotOwnDashboardSchema verifies that the daemon's
// migration authority does not create dashboard-only tables. The dashboard
// runs against a separate database; shared-looking names such as api_keys and
// share_links are intentionally not part of this denylist.
func TestStorageMigrationsDoNotOwnDashboardSchema(t *testing.T) {
	const (
		dashboardUsers        = "users"
		dashboardUserSettings = "user_settings"
		dashboardQuotaAccounts = "quota_accounts"
	)
	for _, migration := range All() {
		for _, statement := range migration.Statements {
			for _, forbidden := range []string{
				...
				dashboardQuotaAccounts,
			} {
				if sqlIdentifierPresent(statement, forbidden) {
					t.Fatalf("migration %d (%s) references dashboard table %q", migration.Version, migration.Name, forbidden)
				}
			}
		}
	}
...
func sqlIdentifierPresent(statement, identifier string) bool {
	tokens := strings.FieldsFunc(strings.ToLower(statement), func(r rune) bool {
...
// TestDropBackupMetadataMigrationRemainsRegistered pins the v2.1 cleanup:
// the backup feature is gone, so its idempotent table drop must remain in the
// storage registry even when newer migrations are appended.
func TestDropBackupMetadataMigrationRemainsRegistered(t *testing.T) {
	for _, migration := range All() {
		if migration.Name != "drop_backup_metadata" {
			continue
		}
		if migration.Version != 31 {
			t.Fatalf("drop_backup_metadata version = %d, want 31", migration.Version)
		}
		if len(migration.Statements) != 1 || !strings.Contains(migration.Statements[0], "DROP TABLE IF EXISTS backup_metadata") {
			t.Fatalf("drop statements = %q", migration.Statements)
		}
		return
	}
	t.Fatal("drop_backup_metadata migration is not registered")
}

func TestFinalMigrationIsBatchJobs(t *testing.T) {
	all := All()
	if len(all) == 0 {
		t.Fatal("migration registry is empty")
	}
	last := all[len(all)-1]
	if last.Version != 32 || last.Name != "batch_jobs" {
		t.Fatalf("final migration = %d (%s), want 32 (batch_jobs)", last.Version, last.Name)
	}
}
