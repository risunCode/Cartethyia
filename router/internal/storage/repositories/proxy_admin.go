...
import (
	...
	admin "github.com/cartethyia/daemon/internal/console/api"
...
// contracts.ProxyRecord. It intentionally omits fields the dashboard does not
...
func proxyAdminRecord(row proxyAdminRow, health string) contracts.ProxyRecord {
	return contracts.ProxyRecord{
		ID:             row.ID,
		Type:           row.Type,
		Host:           row.Host,
		Port:           row.Port,
...
func (r *BunProxyAdminRepository) List(ctx context.Context) ([]contracts.ProxyRecord, error) {
	...
	if len(rows) == 0 {
		return []contracts.ProxyRecord{}, nil
	}
	...
	out := make([]contracts.ProxyRecord, len(rows))
...
func (r *BunProxyAdminRepository) Create(ctx context.Context, input contracts.ProxyInput) (contracts.ProxyRecord, error) {
	if err := r.ready(); err != nil {
		return contracts.ProxyRecord{}, err
	}
	...
	if err != nil {
		return contracts.ProxyRecord{}, fmt.Errorf("proxy admin: create id: %w", err)
	}
	now := time.Now().UTC()
	...
	if _, err := r.db.NewRaw(
		...
		return contracts.ProxyRecord{}, fmt.Errorf("proxy admin: create: %w", err)
	}
...
func (r *BunProxyAdminRepository) Update(ctx context.Context, id string, input contracts.ProxyInput) (contracts.ProxyRecord, error) {
	if err := r.ready(); err != nil {
		return contracts.ProxyRecord{}, err
	}
	id = strings.TrimSpace(id)
	if id == "" {
		return contracts.ProxyRecord{}, errors.New("proxy admin: id is required")
	}
	...
	if err != nil {
		return contracts.ProxyRecord{}, fmt.Errorf("proxy admin: update: %w", err)
	}
	n, _ := res.RowsAffected()
	if n == 0 {
		return contracts.ProxyRecord{}, fmt.Errorf("%w: %w", contracts.ErrNotFound, sql.ErrNoRows)
	}
	return proxyAdminLoad(ctx, r.db, id, "")
}
...
func (r *BunProxyAdminRepository) Delete(ctx context.Context, id string) error {
	...
	n, _ := res.RowsAffected()
	if n == 0 {
		return fmt.Errorf("%w: %w", contracts.ErrNotFound, sql.ErrNoRows)
	}
	return nil
}
...
func proxyAdminLoad(ctx context.Context, db *bun.DB, id, fallbackHealth string) (contracts.ProxyRecord, error) {
	...
	if health == "" {
		...
		if err := db.NewSelect().Model(&h).Where("proxy_id = ?", id).Scan(ctx); err == nil {
			...
			return contracts.ProxyRecord{}, fmt.Errorf("proxy admin: load health: %w", err)
		} else {
			health = "unknown"
		}
	}
...
func proxyAdminValidateInput(input contracts.ProxyInput, requireAll bool) error {
	if requireAll {
