...
VALUES (?, ?, ?, ?, TRUE, ?, ?, ?, NOW())`, id, id, "fixture-secret-"+suffix, "fixture", daily, monthly, lifetime).Exec(ctx)
	...
	reserve := func(t *testing.T, keyID, requestID string, attempt int, window time.Time, estimate int64) router.TokenReservation {
		t.Helper()
		reservation, err := store.TokenBudget.Reserve(ctx, router.ReservationRequest{KeyID: keyID, RequestID: requestID, Attempt: attempt, WindowUTC: window, Estimate: estimate})
		if err != nil {
			t.Fatal(err)
		}
		return reservation
	...
	t.Run("competing reservations are atomic", func(t *testing.T) {
		...
		for attempt := 1; attempt <= 2; attempt++ {
			...
			go func() {
				defer wg.Done()
				<-start
				_, err := store.TokenBudget.Reserve(ctx, router.ReservationRequest{KeyID: keyID, RequestID: "concurrent", Attempt: attempt, WindowUTC: now, Estimate: 60})
				errs <- err
			}()
		}
		close(start)
		...
		for err := range errs {
			...
			case err == nil:
				admitted++
			case errors.Is(err, router.ErrLimit):
				limited++
			default:
	...
	t.Run("retry attempts account independently", func(t *testing.T) {
		...
		if err := first.Release(ctx, router.ReleaseUnaccepted); err != nil {
			t.Fatal(err)
		}
		actual := int64(35)
	...
	t.Run("over estimate refunds and under estimate commits", func(t *testing.T) {
		...
		if _, err := store.TokenBudget.Reserve(ctx, router.ReservationRequest{KeyID: underKey, RequestID: "after-under", Attempt: 1, WindowUTC: now, Estimate: 1}); !errors.Is(err, router.ErrLimit) {
