...
func TestManagerBrowserCallbackIsProviderBoundAndOneShot(t *testing.T) {
	...
	s, err := m.Start("claude", FlowBrowser, "https://example.test/authorize", "", "", 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestManagerExpiresBoundedDeviceSession(t *testing.T) {
	...
	s, err := m.Start("codex", FlowDevice, "", "ABCD", "https://example.test/device", 2)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := m.Start("codex", FlowDevice, "", "EFGH", "https://example.test/device", 2); err == nil {
