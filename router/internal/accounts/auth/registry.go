...
func NewRegistry(configs map[string]Config) (*Registry, error) {
	r := &Registry{drivers: make(map[string]AuthDriver, len(supportedIDs))}
...
func (r *Registry) Get(id string) (AuthDriver, bool) {
	if r == nil {
		return nil, false
	}
...
func defaultConfig(id string) Config {
	base := Config{ProviderID: id, Kind: KindOAuth, ClientID: "cartethyia-public-client", RedirectURI: "http://127.0.0.1/callback", Scopes: []string{"openid", "profile", "email"}, Capabilities: Capabilities{Browser: true, Exchange: true, Refresh: true, Revoke: true}}
	switch id {
	case ProviderClaude:
	...
	case ProviderAntigravity:
		...
		base.ClientSecret = NewSecretFromString(decodeBundledOAuthValue("R09DU1BYLUs1OEZXUjQ4NkxkTEoxbUxCOHNYQzR6NnFEQWY="))
	...
	case ProviderKiro:
		base.Kind = KindDevice
		base.Capabilities = Capabilities{Browser: true, Device: true, Poll: true, Exchange: true, Refresh: true}
	...
	case ProviderKimchi:
		base.Capabilities = Capabilities{Browser: true, Exchange: true, AccessOnly: true}
...
func mergeConfig(base, override Config) Config {
	if strings.TrimSpace(override.ProviderID) != "" { base.ProviderID = override.ProviderID }
	if override.Kind != "" { base.Kind = override.Kind }
	if override.Capabilities != (Capabilities{}) { base.Capabilities = override.Capabilities }
	if override.ClientID != "" { base.ClientID = override.ClientID }
	if override.ClientSecret != nil { base.ClientSecret = override.ClientSecret }
	if override.RedirectURI != "" { base.RedirectURI = override.RedirectURI }
	if len(override.Scopes) > 0 { base.Scopes = append([]string(nil), override.Scopes...) }
	base.Endpoints = mergeEndpoints(base.Endpoints, override.Endpoints)
	if override.HTTPClient != nil { base.HTTPClient = override.HTTPClient }
	if override.Timeout > 0 { base.Timeout = override.Timeout }
	if override.MaxBody > 0 { base.MaxBody = override.MaxBody }
	if override.Now != nil { base.Now = override.Now }
	if override.KiroAWS { base.KiroAWS = true }
	if override.KiroSocialAuthorize != "" { base.KiroSocialAuthorize = override.KiroSocialAuthorize }
	if override.KiroSocialToken != "" { base.KiroSocialToken = override.KiroSocialToken }
	if override.KiroAWSRegion != "" { base.KiroAWSRegion = override.KiroAWSRegion }
	if override.KiroAWSStartURL != "" { base.KiroAWSStartURL = override.KiroAWSStartURL }
	if override.KiroAWSClientName != "" { base.KiroAWSClientName = override.KiroAWSClientName }
	if override.KiroAWSClientType != "" { base.KiroAWSClientType = override.KiroAWSClientType }
	if override.KiroAWSIssuerURL != "" { base.KiroAWSIssuerURL = override.KiroAWSIssuerURL }
	if len(override.KiroAWSGrantTypes) > 0 { base.KiroAWSGrantTypes = append([]string(nil), override.KiroAWSGrantTypes...) }
	if len(override.ExtraAuthorizeParams) > 0 {
		if base.ExtraAuthorizeParams == nil { base.ExtraAuthorizeParams = map[string]string{} }
		for k, v := range override.ExtraAuthorizeParams { base.ExtraAuthorizeParams[k] = v }
	}
	return base
}
func mergeEndpoints(a, b Endpoints) Endpoints {
	if b.Authorize != "" { a.Authorize = b.Authorize }
	if b.Device != "" { a.Device = b.Device }
	if b.Token != "" { a.Token = b.Token }
	if b.Refresh != "" { a.Refresh = b.Refresh }
	if b.UserInfo != "" { a.UserInfo = b.UserInfo }
	if b.Project != "" { a.Project = b.Project }
	if b.Revoke != "" { a.Revoke = b.Revoke }
	return a
}
func normalizeID(id string) string {
	id = strings.ToLower(strings.TrimSpace(id))
	if id == "grok" { return ProviderGrokBuild }
	if id == "cline-pass" { return ProviderClinePass }
	return id
}

// Definitions returns the provider OAuth descriptors owned by auth.
func Definitions() []ProviderDefinition {
	return []ProviderDefinition{Antigravity(), ClaudeCode(), Cline(), ClinePass(), Codex(), GrokBuild(), Kimchi(), Kiro()}
}
