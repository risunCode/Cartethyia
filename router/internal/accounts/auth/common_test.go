...
func fixtureConfig(id string, caps Capabilities, tr *fixtureTransport) Config {
...
func TestBrowserPKCEAndRotatedRefreshFallback(t *testing.T) {
	tr := &fixtureTransport{}
	d, err := New(fixtureConfig("claude", Capabilities{Browser: true, Exchange: true, Refresh: true}, tr))
	if err != nil {
		t.Fatal(err)
	}
	start, err := d.Start(context.Background(), OAuthStartInput{Flow: FlowBrowser})
	if err != nil {
		t.Fatal(err)
	}
	...
	ts, err := d.Exchange(context.Background(), OAuthExchangeInput{Code: "auth-code", State: start.State})
	if err != nil {
		t.Fatal(err)
	}
	...
	ts2, err := d.Refresh(context.Background(), RefreshTokenInput{AccountID: "acct", RefreshToken: ts.Refresh})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDevicePendingThenCompleteAndAccessOnlyReauth(t *testing.T) {
	...
	d, err := New(fixtureConfig("codex", Capabilities{Device: true, Poll: true, Refresh: true}, tr))
	if err != nil {
		t.Fatal(err)
	}
	start, err := d.Start(context.Background(), OAuthStartInput{Flow: FlowDevice})
	if err != nil {
		t.Fatal(err)
	}
	pending, err := d.Poll(context.Background(), start.State)
	if err != nil || pending.Status != PollPending {
		t.Fatalf("pending = %#v %v", pending, err)
	}
	complete, err := d.Poll(context.Background(), start.State)
	if err != nil || complete.Status != PollCompleted || complete.TokenSet == nil {
		t.Fatalf("complete = %#v %v", complete, err)
	}
	complete.TokenSet.Close()
	kimchi, err := New(fixtureConfig("kimchi", Capabilities{Browser: true, Exchange: true, AccessOnly: true}, tr))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := kimchi.Refresh(context.Background(), RefreshTokenInput{AccountID: "acct", RefreshToken: NewSecretFromString("refresh")}); Classify(err) != ErrKindReauthentication {
...
func TestRegistryCanonicalIDsAndExclusions(t *testing.T) {
	...
	if d, ok := r.Get("grok"); !ok || d.(DriverInfo).ID() != "grok-build" {
