...
func check(t *testing.T, d AuthDriver, err error) {
	t.Helper()
	if err != nil {
...
func TestDriverRevoke(t *testing.T) {
	...
	d := &HTTPDriver{
		cfg: Config{
			ProviderID:   "custom",
			Capabilities: Capabilities{Revoke: true},
	...
	sec := NewSecret([]byte("token-to-revoke"))
	if err := d.Revoke(context.Background(), RevokeTokenInput{Token: sec}); err != nil {
		t.Fatal(err)
	}
	if !called {
	...
	if err := dNoRevoke.Revoke(context.Background(), RevokeTokenInput{Token: sec}); err == nil {
