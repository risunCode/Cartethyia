// Package auth contains bounded interactive OAuth orchestration primitives.
...
var (
	...
	ErrProviderDenied          = errors.New("oauth flow: provider denied authorization")
)
...
func (m *Manager) Start(providerID string, flow OAuthFlowKind, authorizationURL, userCode, verificationURI string, intervalSeconds int) (Session, error) {
	providerID = strings.TrimSpace(providerID)
	if providerID == "" || len(providerID) > maxTokenBytes {
		return Session{}, errors.New("oauth flow: provider id is required")
	}
	if flow != FlowBrowser && flow != FlowDevice {
	...
	now := m.now()
	s := &Session{ID: id, ProviderID: providerID, Flow: flow, State: state, CodeVerifier: verifier, AuthorizationURL: boundedSessionValue(authorizationURL, 2048), UserCode: boundedSessionValue(userCode, 128), VerificationURI: boundedSessionValue(verificationURI, 2048), IntervalSeconds: clampInterval(intervalSeconds), ExpiresAt: now.Add(m.ttl), Status: StatusPending}
	m.sessions[id] = s
...
func (m *Manager) StartFromOAuth(providerID string, flow OAuthFlowKind, result OAuthStartResult) (Session, error) {
	providerID = strings.TrimSpace(providerID)
	if providerID == "" || len(providerID) > maxTokenBytes {
		return Session{}, errors.New("oauth flow: provider id is required")
	}
	...
	state := boundedSessionValue(result.State, maxTokenBytes)
	if state == "" {
		var err error
		state, err = randomToken(32)
		if err != nil {
	...
	verifier := boundedSessionValue(result.CodeVerifier, maxTokenBytes)
	if flow == FlowBrowser && verifier == "" {
		var err error
		verifier, err = randomToken(48)
		if err != nil {
			return Session{}, err
	...
	s := &Session{
		...
		AuthorizationURL: boundedSessionValue(result.AuthorizationURL, 2048),
		UserCode:         boundedSessionValue(result.UserCode, 128),
		VerificationURI:  boundedSessionValue(result.VerificationURI, 2048),
...
func (m *Manager) ConsumeCallback(id, providerID, callbackURL string) (Session, error) {
	...
	s.Consumed = true
	s.Code = boundedSessionValue(code, maxTokenBytes)
...
func (m *Manager) ConsumeCallbackForExchange(id, providerID, callbackURL, redirectURI string) (Session, OAuthExchangeInput, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	s, err := m.lookupLocked(id, providerID)
	if err != nil {
		return Session{}, OAuthExchangeInput{}, err
	}
	if s.Consumed {
		return Session{}, OAuthExchangeInput{}, ErrSessionConsumed
	}
	code, state, providerError, err := ParseLoopbackCallback(callbackURL)
	if err != nil || state != s.State {
		return Session{}, OAuthExchangeInput{}, ErrInvalidCallback
	}
	if providerError != "" {
		s.Status = StatusDenied
		s.Consumed = true
		return publicCopy(*s), OAuthExchangeInput{}, errors.New("oauth flow: provider denied authorization")
	}
	if code == "" {
		return Session{}, OAuthExchangeInput{}, ErrInvalidCallback
	}
	...
	s.CallbackState = state
	s.Status = StatusCompleted
	return publicCopy(*s), OAuthExchangeInput{
		ProviderID:   providerID,
		Code:         s.Code,
		State:        s.State,
...
func (m *Manager) ConsumeCodeForExchange(id, providerID, code, state, redirectURI string) (Session, OAuthExchangeInput, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	s, err := m.lookupLocked(id, providerID)
	if err != nil {
...
func boundedSessionValue(s string, max int) string {
