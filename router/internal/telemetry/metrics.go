...
type Registry struct {
	...
	batch      MaintenanceBatchWriter

	// maintenanceBatch is the registry-owned snapshot for batch lifecycle
	// metrics. Writers may expose a live MaintenanceBatchStats snapshot; the
	// explicit observation methods remain useful for other implementations and
	// keep the registry as the single metrics authority.
	maintenanceBatchMu sync.RWMutex
	maintenanceBatch   MaintenanceBatchStats
...
// MaintenanceBatchStats is the bounded, payload-free registry snapshot used
// for maintenance batch telemetry scraping. Durations are represented as
// time.Duration in-process and rendered in stable units by ServeHTTP.
//
// QueueDepth, QueueSize, and FlushSize are gauges. Drops, Failures, and
// Flushes are cumulative counters. Draining and Drained expose shutdown
// progress.
type MaintenanceBatchStats struct {
	QueueDepth    int
	QueueSize     int
	FlushSize     int
...
// maintenanceBatchStatsSource is intentionally optional. The locked
// MaintenanceBatchWriter contract remains the enqueue/drop/failure/close
// boundary plus queue capacity; writers that expose flush and lifecycle
// accessors are scraped without adding another metrics authority.
type maintenanceBatchStatsSource interface {
	QueueDepth() int
	Flushes() uint64
	FlushLatency() time.Duration
	Draining() bool
...
type maintenanceBatchFlushConfigSource interface {
	FlushSize() int
	FlushInterval() time.Duration
}
...
// WithMetadataWriter is the compatibility wiring for the existing bounded
// asynchronous metadata writer. Its batch queue and persistence lifecycle are
// reflected in the registry's metadata-batch metrics.
func (r *Registry) WithMetadataWriter(writer *AsyncMetadataWriter) *Registry {
	r.metadata = writer
	return r.WithMaintenanceBatchWriter(writer)
}

// WithMaintenanceBatchWriter connects the bounded maintenance batch writer to
// the existing registry. It does not add a second metrics authority: the
// writer supplies only its current lifecycle snapshot and the Registry owns
// metric rendering.
func (r *Registry) WithMaintenanceBatchWriter(writer MaintenanceBatchWriter) *Registry {
	if r == nil {
		return r
	}
	r.batch = writer
	if metadata, ok := writer.(*AsyncMetadataWriter); ok {
		r.metadata = metadata
	}
	return r
}

// ObserveMaintenanceBatchQueue records the latest queue depth and configured
// flush size/interval without blocking the request path.
func (r *Registry) ObserveMaintenanceBatchQueue(depth, flushSize int, flushInterval time.Duration) {
	if r == nil {
		return
	}
	r.maintenanceBatchMu.Lock()
	r.maintenanceBatch.QueueDepth = maxNonNegative(depth)
	r.maintenanceBatch.FlushSize = maxNonNegative(flushSize)
	if flushInterval < 0 {
		flushInterval = 0
	}
	r.maintenanceBatch.FlushInterval = flushInterval
	r.maintenanceBatchMu.Unlock()
}

// ObserveMaintenanceBatchFlush records one completed flush outcome.
func (r *Registry) ObserveMaintenanceBatchFlush(size int, latency time.Duration, failed bool) {
	if r == nil {
		return
	}
	if size < 0 {
	...
	r.maintenanceBatchMu.Lock()
	r.maintenanceBatch.Flushes++
	r.maintenanceBatch.FlushLatency = latency
	if failed {
		r.maintenanceBatch.Failures++
	}
	r.maintenanceBatchMu.Unlock()
}

// ObserveMaintenanceBatchDrop records one queue or retry drop.
func (r *Registry) ObserveMaintenanceBatchDrop() {
	if r == nil {
		return
	}
	r.maintenanceBatchMu.Lock()
	r.maintenanceBatch.Drops++
	r.maintenanceBatchMu.Unlock()
}

// ObserveMaintenanceBatchDrain records shutdown progress and outcome.
func (r *Registry) ObserveMaintenanceBatchDrain(draining, drained bool) {
	if r == nil {
		return
	}
	r.maintenanceBatchMu.Lock()
	r.maintenanceBatch.Draining = draining
	r.maintenanceBatch.Drained = drained
	r.maintenanceBatchMu.Unlock()
}
...
func (r *Registry) ServeHTTP(w http.ResponseWriter) {
	...
	batch := r.maintenanceBatchSnapshot()
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_queue_depth %d\n", batch.QueueDepth)
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_queue_size %d\n", batch.QueueSize)
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_flush_size %d\n", batch.FlushSize)
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_flush_interval_seconds %g\n", batch.FlushInterval.Seconds())
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_drops_total %d\n", batch.Drops)
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_failures_total %d\n", batch.Failures)
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_flush_total %d\n", batch.Flushes)
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_flush_latency_ms %g\n", float64(batch.FlushLatency)/float64(time.Millisecond))
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_draining %d\n", boolMetric(batch.Draining))
	_, _ = fmt.Fprintf(w, "cartethyia_metadata_batch_drained %d\n", boolMetric(batch.Drained))
...
func (r *Registry) maintenanceBatchSnapshot() MaintenanceBatchStats {
	if r == nil {
		return MaintenanceBatchStats{}
	}
	r.maintenanceBatchMu.RLock()
	snapshot := r.maintenanceBatch
	r.maintenanceBatchMu.RUnlock()
	if r.batch == nil {
		return snapshot
	}
	if source, ok := r.batch.(maintenanceBatchStatsSource); ok {
		snapshot.QueueDepth = maxNonNegative(source.QueueDepth())
		snapshot.Flushes = source.Flushes()
		snapshot.FlushLatency = nonNegativeDuration(source.FlushLatency())
		snapshot.Draining = source.Draining()
		snapshot.Drained = source.Drained()
	}
	snapshot.QueueSize = maxNonNegative(r.batch.QueueSize())
	if source, ok := r.batch.(maintenanceBatchFlushConfigSource); ok {
		snapshot.FlushSize = maxNonNegative(source.FlushSize())
		snapshot.FlushInterval = nonNegativeDuration(source.FlushInterval())
	}
	snapshot.Drops = r.batch.Drops()
	snapshot.Failures = r.batch.Failures()
	return snapshot
}
