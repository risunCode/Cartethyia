...
func (s *registryCatalogAdminService) Providers(ctx context.Context) ([]CatalogProvider, error) {
	...
	items := make([]CatalogProvider, 0, s.registry.Size())
	for _, id := range s.registry.IDs() {
		...
		items = append(items, CatalogProvider{
			ID:              meta.ID,
...
func adminCatalogModels(providerID string, provider providers.Provider, generation uint64) []CatalogModel {
	entries := provider.Models()
	if entries == nil {
		return nil
	}
	declared := entries.List()
	out := make([]CatalogModel, 0, len(declared))
...
var _ CatalogService = (*registryCatalogAdminService)(nil)
