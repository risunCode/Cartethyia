...
func newSessionAuthService(repository *dbrepositories.BunSettingsRepository, oauth OAuthLifecycleService, bootstrapPassword string) *sessionAuthService {
...
// cookie.
func (s *sessionAuthService) Login(ctx context.Context, input LoginInput, request AuthRequest) (LoginResult, error) {
	if s == nil || s.settings == nil {
		return LoginResult{}, NewError(CodeAdminUnavailable, "console auth is not configured")
	}
	ip := boundedLoginKey(request.IP)
	if wait, limited := s.limiter.retryAfter(ip); limited {
		return LoginResult{}, NewError(CodeRateLimited, fmt.Sprintf("too many failed attempts; retry after %ds", int(wait.Seconds())+1))
	}

	settings, err := s.currentSettings(ctx)
	if err != nil {
		return LoginResult{}, Wrap(CodeAdminUnavailable, "console settings are unavailable: "+err.Error(), err)
	}
	settings, err = s.ensureConsoleBootstrap(ctx, settings)
	if err != nil {
		return LoginResult{}, Wrap(CodeAdminUnavailable, "console bootstrap is unavailable", err)
	}
	if strings.TrimSpace(settings.PasswordHash) == "" {
		return LoginResult{}, NewError(CodeAdminAuthentication, "console password is not initialized")
	}
	if !verifyConsolePassword(settings.PasswordHash, input.Password) {
		s.limiter.recordFailure(ip)
		return LoginResult{}, NewError(CodeAdminAuthentication, "invalid credentials")
	}
	s.limiter.recordSuccess(ip)

	secret, err := s.ensureSessionSecret(ctx, settings)
	if err != nil {
		return LoginResult{}, Wrap(CodeAdminUnavailable, "session signing key is unavailable", err)
	}
	...
	if err != nil {
		return LoginResult{}, Wrap(CodeAdminUnavailable, "session token signing failed", err)
	}
...
func (s *sessionAuthService) Current(ctx context.Context, sessionID string) (Session, error) {
	if s == nil || s.settings == nil {
		return Session{}, NewError(CodeAdminAuthentication, "authentication required")
	}
	settings, err := s.currentSettings(ctx)
	if err != nil {
		return Session{}, Wrap(CodeAdminUnavailable, "console settings are unavailable: "+err.Error(), err)
	}
	secret, err := s.ensureSessionSecret(ctx, settings)
	if err != nil {
		return Session{}, Wrap(CodeAdminUnavailable, "session signing key is unavailable", err)
	}
...
// session shape.
func (s *sessionAuthService) Refresh(ctx context.Context, sessionID string, request AuthRequest) (LoginResult, error) {
	if s == nil || s.settings == nil {
...
func requestSecure(request AuthRequest) bool {
