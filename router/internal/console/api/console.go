...
func RegisterConsole(mux *http.ServeMux, services Services) {
	if services.ConsoleLogs != nil {
		...
		mux.HandleFunc("/console/logs", requireMethod(http.MethodGet, func(w http.ResponseWriter, r *http.Request) {
			query := consolecontracts.ConsoleLogQuery{
		...
		mux.HandleFunc("/console/client-errors", requireMethod(http.MethodPost, func(w http.ResponseWriter, r *http.Request) {
			var input consolecontracts.ClientErrorInput
			if err := decodeJSON(r, &input); err != nil {
				WriteError(w, NewError(CodeInvalidRequest, "invalid client error body").WithCause(err))
				return
			}
			if err := logs.Insert(r.Context(), input); err != nil {
				WriteError(w, err)
				return
			}
...
func tailConsoleLogs(r *http.Request, stream *adminStream, logs ConsoleLogService) {
	...
	ctx := r.Context()
	entries, err := logs.List(ctx, consolecontracts.ConsoleLogQuery{Level: level, Scope: scope, Limit: limit})
	if err != nil {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			tail, err := logs.List(ctx, consolecontracts.ConsoleLogQuery{From: newest, Level: level, Scope: scope, Limit: adminStreamTailLimit})
			if err != nil {
...
func emitConsoleEntries(stream *adminStream, entries []consolecontracts.ConsoleLogEntry, newest string) string {
	sorted := make([]consolecontracts.ConsoleLogEntry, len(entries))
	copy(sorted, entries)
...
func consoleStreamSource(entry consolecontracts.ConsoleLogEntry) string {
	if entry.Scope != "" {
		return entry.Scope
	}
	if entry.Provider != "" {
