...
func RegisterAuth(mux *http.ServeMux, services Services) {
	...
	mux.HandleFunc("/console/auth/oauth/reauth", requireMethod(http.MethodPost, func(w http.ResponseWriter, r *http.Request) {
		...
		input.Force = true
		var (
			state consolecontracts.OAuthState
			err   error
		)
...
func writeOAuthState(w http.ResponseWriter, state consolecontracts.OAuthState) {
...
func clientIP(r *http.Request) string {
	trusted, err := strconv.ParseBool(strings.TrimSpace(os.Getenv("CARTETHYIA_TRUST_PROXY")))
	return middleware.ClientKeyWithTrust(r, err == nil && trusted)
}
