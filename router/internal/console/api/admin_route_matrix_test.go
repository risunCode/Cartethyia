...
func (a matrixAuthService) Login(_ context.Context, _ LoginInput, req AuthRequest) (LoginResult, error) {
	_ = req
	return LoginResult{
		Session:   consolecontracts.Session{ID: "session-1", User: "operator", Scopes: []string{"admin:*"}},
...
func (a matrixAuthService) Current(_ context.Context, sessionID string) (consolecontracts.Session, error) {
	if sessionID == "" {
		return consolecontracts.Session{}, NewError(CodeAdminAuthentication, "authentication required")
	}
	scopes := a.scopes
	if len(scopes) == 0 {
		scopes = []string{"admin:*"}
	}
	return consolecontracts.Session{ID: sessionID, User: "operator", Scopes: scopes}, nil
}
...
func (a matrixAuthService) OAuthStart(context.Context, string, OAuthStartInput) (consolecontracts.OAuthState, error) {
	return consolecontracts.OAuthState{SessionID: "oauth-session", Status: "pending", URL: "https://example.test/authorize", Flow: "browser"}, nil
}
func (a matrixAuthService) OAuthComplete(context.Context, string, OAuthCompleteInput) (consolecontracts.OAuthState, error) {
	return consolecontracts.OAuthState{SessionID: "oauth-session", Status: "completed", AccountID: "acct-1"}, nil
}
func (a matrixAuthService) OAuthCancel(context.Context, string) error { return nil }
func (a matrixAuthService) OAuthRefresh(_ context.Context, input OAuthRefreshInput) (consolecontracts.OAuthState, error) {
	return consolecontracts.OAuthState{AccountID: input.AccountID, Status: "refreshed"}, nil
}
func (a matrixAuthService) OAuthStatus(context.Context, string) (consolecontracts.OAuthState, error) {
	return consolecontracts.OAuthState{SessionID: "oauth-session", Status: "pending", UserCode: "ABCD", VerificationURI: "https://example.test/device"}, nil
}
func (a matrixAuthService) OAuthReauthenticate(_ context.Context, input OAuthRefreshInput) (consolecontracts.OAuthState, error) {
...
func (matrixAccounts) List(context.Context, string) ([]consolecontracts.Account, error) {
	return []consolecontracts.Account{{ID: "acct-1", Provider: "openai", Enabled: true}}, nil
}
func (matrixAccounts) BatchCreate(_ context.Context, _ string, items []consolecontracts.AccountInput) ([]consolecontracts.Account, error) {
	out := make([]consolecontracts.Account, 0, len(items))
	for i := range items {
		out = append(out, consolecontracts.Account{ID: "batch-" + items[i].Label, Provider: "openai", Enabled: true})
	}
	return out, nil
}
func (matrixAccounts) Create(_ context.Context, providerID string, input consolecontracts.AccountInput) (consolecontracts.Account, error) {
	return consolecontracts.Account{ID: "acct-new", Provider: providerID, Name: input.Name, Enabled: true}, nil
}
func (matrixAccounts) Update(_ context.Context, providerID, accountID string, input consolecontracts.AccountInput) (consolecontracts.Account, error) {
	return consolecontracts.Account{ID: accountID, Provider: providerID, Name: input.Name, Enabled: true}, nil
}
func (matrixAccounts) Delete(context.Context, string, string) error { return nil }
func (matrixAccounts) BatchDelete(context.Context, string, []string) (consolecontracts.BatchResult, error) {
	return consolecontracts.BatchResult{Processed: 1, Succeeded: 1}, nil
}
func (matrixAccounts) BatchUpdate(context.Context, string, []consolecontracts.AccountBatchPatch) (consolecontracts.BatchResult, error) {
	return consolecontracts.BatchResult{Processed: 1, Succeeded: 1}, nil
}
...
func (matrixAccounts) RefreshQuota(_ context.Context, accountID string) (consolecontracts.QuotaState, error) {
	return consolecontracts.QuotaState{AccountID: accountID, Used: 1, Limit: 10, Remaining: 9}, nil
}
func (matrixAccounts) Quota(_ context.Context, accountID string) (consolecontracts.QuotaState, error) {
	return consolecontracts.QuotaState{AccountID: accountID, Used: 1, Limit: 10, Remaining: 9}, nil
}
...
func TestAdminRouteMatrixAuthenticatedSuccessAndAuthFailure(t *testing.T) {
	...
	success := []caseSpec{
		...
		{group: "console", method: http.MethodPost, path: "/console/client-errors", body: `{"level":"error","message":"boom"}`, want: http.StatusNoContent},
	...
	failures := []caseSpec{
		...
		{group: "console", method: http.MethodPost, path: "/console/client-errors", want: http.StatusUnauthorized},
...
func TestAdminRouteMatrixSubresourceCoverage(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "method_not_allowed_accounts", method: http.MethodPut, path: "/console/accounts", want: http.StatusMethodNotAllowed},
		{name: "method_not_allowed_dashboard", method: http.MethodPost, path: "/console/dashboard", want: http.StatusMethodNotAllowed},
		{name: "method_not_allowed_client_errors_get", method: http.MethodGet, path: "/console/client-errors", want: http.StatusMethodNotAllowed},
		...
		{name: "client_errors_invalid_level", method: http.MethodPost, path: "/console/client-errors", body: `{"level":"verbose","message":"boom"}`, want: http.StatusBadRequest},
	}
...
func TestAdminRouteMatrixSessionAuthorizerAndScopes(t *testing.T) {
	...
	usageDenied := withSession(httptest.NewRequest(http.MethodPost, "/console/client-errors", strings.NewReader(`{"level":"error","message":"boom"}`)))
	usageDenied.Header.Set("Content-Type", "application/json")
	usageRec := httptest.NewRecorder()
	mux.ServeHTTP(usageRec, usageDenied)
	if usageRec.Code != http.StatusForbidden {
		t.Fatalf("usage denied status=%d body=%s", usageRec.Code, usageRec.Body.String())
	}
}
...
func TestAdminRouteMatrixEnvelopeAndValidationHelpers(t *testing.T) {
	...
	if err := validateAdminPayload(&consolecontracts.AccountInput{Label: strings.Repeat("x", 300)}); err == nil {
		t.Fatal("expected account validation error")
	}
	listen := "not-a-listen-addr"
	if err := validateAdminPayload(&consolecontracts.RuntimeSettingsInput{ListenAddr: &listen}); err == nil {
	...
	req.Header.Set("X-consolecontracts.Session-Id", "from-header")
	if got := readSessionID(req); got != "" {
...
func TestAdminRouteMatrixNilServiceRegistrationIsAbsent(t *testing.T) {
	...
	req = httptest.NewRequest(http.MethodGet, "/console/client-errors", nil)
	rec = httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
	if rec.Code != http.StatusNotFound {
		t.Fatalf("nil console logs should leave client-errors unregistered, got %d", rec.Code)
	}
}
