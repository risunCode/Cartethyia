...
func (s catalogTestService) Providers(context.Context) ([]consolecontracts.CatalogProvider, error) {
	if s.providerErr != nil {
		return nil, s.providerErr
	}
	return []consolecontracts.CatalogProvider{{
		ID:              " openai ",
		DisplayName:     " OpenAI ",
		Protocol:        "openai",
		...
		Models: []consolecontracts.CatalogModel{{
...
func TestCatalogRoutesReturnRedactedEnvelopeData(t *testing.T) {
	...
	var envelope struct {
		Data struct {
			Items []consolecontracts.CatalogProvider `json:"items"`
		} `json:"data"`
	}
