...
func handleShareMonitor(w http.ResponseWriter, r *http.Request, opts *ShareOptions, token string) {
	...
	response := consolecontracts.ShareMonitorResponse{
		Name: key.Name, Active: key.Active, APIKey: consolecontracts.ShareAPIKeyResponse{ID: key.ID, Prefix: key.KeyPrefix, Active: key.Active},
		...
		Notes:     consolecontracts.ShareNotes{Title: key.QuoteBigText, Subtitle: key.QuoteSubText, Body: key.QuoteBody},
...
func handleShareSetup(w http.ResponseWriter, r *http.Request, opts *ShareOptions, token string) {
	...
	if !ok {
		...
		if models.IsNotFound(err) || candidate.ID == "" || candidate.Kind != "setup" {
			writeError(w, http.StatusNotFound, "link_not_found")
			return
		}
	...
	writeJSON(w, http.StatusOK, consolecontracts.ShareSetupResponse{Name: key.Name, Key: credential, BaseURL: requestBaseURL(r), ExpiresAt: link.ExpiresAt})
}
