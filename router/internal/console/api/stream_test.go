...
type streamConsoleLogs struct {
	mu      sync.Mutex
	entries []consolecontracts.ConsoleLogEntry
}

func (l *streamConsoleLogs) List(context.Context, consolecontracts.ConsoleLogQuery) ([]consolecontracts.ConsoleLogEntry, error) {
	l.mu.Lock()
	defer l.mu.Unlock()
	return append([]consolecontracts.ConsoleLogEntry(nil), l.entries...), nil
}

func (l *streamConsoleLogs) Insert(context.Context, consolecontracts.ClientErrorInput) error {
	return nil
}

func (l *streamConsoleLogs) append(entries ...consolecontracts.ConsoleLogEntry) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func TestAdminConsoleLogStreamInitialAndTail(t *testing.T) {
	...
	logs := &streamConsoleLogs{entries: []consolecontracts.ConsoleLogEntry{
	...
	logs.append(consolecontracts.ConsoleLogEntry{ID: "c", Timestamp: "2026-08-16T10:00:02Z", Level: "error", Scope: "upstream", Message: "third"})
	events = readStreamEvents(t, reader, 1)
...
func TestAdminStreamSkipsBufferedAuditPath(t *testing.T) {
	...
	defer resp.Body.Close()
	reader := bufio.NewReader(resp.Body)
	logs.append(consolecontracts.ConsoleLogEntry{ID: "x", Timestamp: "2026-08-16T11:00:00Z", Level: "info", Scope: "s", Message: "flushed"})
