...
func RegisterCatalog(mux *http.ServeMux, services Services) {
	mux.HandleFunc(CatalogProvidersPath, requireMethod(http.MethodGet, func(w http.ResponseWriter, r *http.Request) {
		...
		safe := make([]consolecontracts.CatalogProvider, 0, len(items))
...
func redactCatalogProvider(provider consolecontracts.CatalogProvider) consolecontracts.CatalogProvider {
	...
	models := make([]consolecontracts.CatalogModel, 0, len(provider.Models))
...
func redactCatalogModel(model consolecontracts.CatalogModel) consolecontracts.CatalogModel {
