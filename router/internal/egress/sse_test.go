...
func TestSSEDecoderPreservesRawFrameDelimiters(t *testing.T) {
	input := []byte(": keepalive\r\nevent: response.output_text.delta\r\ndata: {\"delta\":\"x\"}\r\n\r\n")
	decoder := newSSEDecoder(bytes.NewReader(input), 256, 1024)
	event, err := decoder.Next()
	if err != nil {
		t.Fatalf("Next() error = %v", err)
	}
	if !bytes.Equal(event.Raw, input) {
		t.Fatalf("raw frame = %q, want exact input %q", event.Raw, input)
	}
}

func TestHTTPTransportPassForwardsUnknownFrameWithoutCanonicalDecode(t *testing.T) {
	const frame = "event: provider.unknown\r\ndata: {\"type\":\"provider.unknown\",\"unknown\":{\"keep\":true}}\r\n\r\n"
	...
	decision, err := contracts.NewCompatibilityDecision(contracts.CompatibilityDecision{
		Mode:              contracts.ModePass,
		SourceSurface:     contracts.SurfaceAnthropic,
		TargetSurface:     contracts.SurfaceAnthropic,
		SourceStream:      true,
		TargetStream:      true,
		CatalogGeneration: 1,
		CapabilityVersion: 1,
	...
	ctx := proxy.WithCompatibilityDecision(context.Background(), decision)
	stream, err := transport.CallStream(ctx, streamRegressionAccount(), streamRegressionRequest())
	if err != nil {
	...
	if !bytes.Equal(event.RawFrame, []byte(frame)) {
		t.Fatalf("raw frame = %q, want %q", event.RawFrame, frame)
	}
	if event.Kind != proxy.EventTextDelta {
		t.Fatalf("raw event kind = %q, want text delta marker", event.Kind)
	}
	terminal, err := stream.Next(context.Background())
	if err != nil {
		t.Fatalf("terminal event error = %v", err)
	}
	if !terminal.IsTerminal() || !bytes.Equal(terminal.RawFrame, []byte("data: [DONE]\r\n\r\n")) {
		t.Fatalf("terminal = %#v, want one raw done frame", terminal)
	}
}
