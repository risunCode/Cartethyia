...
func NewDurableProxyCoordinator(repository dbrepositories.ProxyRepository, evidence *telemetry.Registry) *durableProxyCoordinator {
	if repository == nil {
		return nil
	}
...
func (s *durableProxyCoordinator) SelectProxy(ctx context.Context, providerID, _ string) (ProxySelection, error) {
	...
	if err != nil || !selection.UseProxy {
		return ProxySelection{Release: selection.Release}, err
	}
	for _, route := range selectionRoutes {
		...
		if parseErr != nil {
			selection.Release()
			return ProxySelection{}, parseErr
		}
	...
	selection.Release()
	return ProxySelection{}, errors.New("egress: selected proxy route disappeared")
}
