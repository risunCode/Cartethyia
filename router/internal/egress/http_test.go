...
func TestHTTPTransportClientForProxy(t *testing.T) {
	...
	cHTTPAgain, err := tr.clientForProxy(httpURL)
	if err != nil || cHTTPAgain != cHTTP {
		t.Fatalf("http proxy client was not reused: first=%p second=%p err=%v", cHTTP, cHTTPAgain, err)
	}
