...
func (t *HTTPTransport) CallStream(ctx context.Context, acct proxy.Account, req contracts.Request) (*proxy.Stream, error) {
	...
	maxResponseBytes := t.maxBytes()
	maxSSELineBytes := t.maxSSELineBytes()
	maxSSEEventBytes := t.maxSSEEventBytes()
	...
	decision, hasDecision := proxy.CompatibilityDecisionFromContext(requestCtx)
	rawPass := hasDecision &&
		decision.Mode == contracts.ModePass &&
		decision.SourceSurface == decision.TargetSurface &&
		decision.SourceSurface == req.Protocol &&
	...
	go func() {
		...
		decoder := newSSEDecoder(resp.Body, maxSSELineBytes, maxSSEEventBytes)
		for {
			...
			if isSSEDone(event.Data) {
				outcome.success(requestCtx)
				if rawPass {
					_ = send(proxy.StreamEvent{Kind: proxy.EventMessageStop, Reason: "completed", RawFrame: event.Raw})
				} else {
			...
			if rawPass {
				terminal, reason, usage := rawTerminalTap(event)
				if usage != nil && !send(proxy.StreamEvent{Kind: proxy.EventUsage, Usage: usage}) {
					return
				}
				if terminal {
					if !send(proxy.StreamEvent{Kind: proxy.EventMessageStop, Reason: reason, RawFrame: event.Raw}) {
						return
					}
					outcome.success(requestCtx)
					return
				}
				if !send(proxy.StreamEvent{Kind: proxy.EventTextDelta, RawFrame: event.Raw}) {
					return
				}
				continue
			}
			...
			if len(mapped) == 0 {
				// Preserve provider-native events that have no canonical
				// projection. A normal bridge ignores this non-semantic event;
				// a PASS bridge forwards its original frame.
				if !send(proxy.StreamEvent{Kind: proxy.EventRawFrame, RawFrame: event.Raw}) {
					return
				}
				continue
			}
			...
			for index := range mapped {
				if mapped[index].IsTerminal() {
					rawIndex = index
					break
				}
			}
			for index, canonical := range mapped {
				if index == rawIndex {
					canonical.RawFrame = event.Raw
				}
				if !send(canonical) {
					return
				}
...
// rawTerminalTap inspects only bounded terminal metadata. It never builds a
// canonical response event and does not alter the raw frame forwarded to the
// client.
func rawTerminalTap(event sseEvent) (bool, string, *proxy.StreamUsage) {
	switch event.Event {
	case "message_stop":
	...
	case "message_delta":
		...
		if json.Unmarshal(event.Data, &payload) == nil && payload.Delta.StopReason != nil {
			return true, canonicalRawReason(*payload.Delta.StopReason), rawUsageTap(event.Data)
		}
	case "response.completed":
		return true, "completed", rawUsageTap(event.Data)
	case "response.incomplete":
		return true, "length", rawUsageTap(event.Data)
	...
	for _, choice := range payload.Choices {
		if choice.FinishReason != nil && *choice.FinishReason != "" {
			return true, canonicalRawReason(*choice.FinishReason), rawUsageTap(event.Data)
		}
	}
	return false, "", nil
}

func canonicalRawReason(reason string) string {
	switch reason {
...
func rawUsageTap(data []byte) *proxy.StreamUsage {
	var payload struct {
	...
	var usage struct {
		...
		CachedRead    *int `json:"cache_read_input_tokens"`
		CachedWrite   *int `json:"cache_creation_input_tokens"`
	...
	if input == nil && output == nil && usage.Total == nil && usage.CachedRead == nil && usage.CachedWrite == nil && usage.Reasoning == nil {
		return nil
	}
	out := &proxy.StreamUsage{}
	if input != nil {
	...
	if usage.CachedRead != nil {
		out.CacheReadTokens = *usage.CachedRead
	}
	if usage.CachedWrite != nil {
		out.CacheWriteTokens = *usage.CachedWrite
	}
	if usage.Reasoning != nil {
		out.ReasoningTokens = *usage.Reasoning
	}
	return out
}
...
func (t *HTTPTransport) clientForProxy(proxyURL *url.URL) (*http.Client, error) {
	...
	t.proxyClientsMu.Lock()
	defer t.proxyClientsMu.Unlock()
	if client, ok := t.proxyClients[cacheKey]; ok {
		return client, nil
	}
	base := t.Client
	...
	if t.proxyClients == nil {
		t.proxyClients = make(map[string]*http.Client)
	}
	t.proxyClients[cacheKey] = base
	return base, nil
}
func (t *HTTPTransport) client() *http.Client {
	t.clientOnce.Do(func() {
		client := t.Client
		if client == nil {
			client = &http.Client{Transport: t.defaultTransport(), Timeout: t.TotalTimeout}
		}
		if t.OutboundPolicy != nil {
...
func (t *HTTPTransport) defaultTransport() *http.Transport {
