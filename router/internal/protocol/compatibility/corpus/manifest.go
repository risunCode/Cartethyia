...
// ComparisonMode identifies the contract used to compare a fixture result.
// Semantic fixtures compare the normalized protocol contract; error fixtures
// compare the bounded client-visible error contract.
type ComparisonMode string

const (
	ComparisonSemantic ComparisonMode = "semantic"
	ComparisonError    ComparisonMode = "error"
)
...
func ValidateManifest(manifest Manifest) error {
	manifest = normalizeComparisonModes(manifest)
	...
	for i := range manifest.Fixtures {
		...
		if spec.ComparisonMode != ComparisonSemantic && spec.ComparisonMode != ComparisonError {
			return corpusError(CodeManifestInvalid, StageManifest, field+".comparison_mode", nil)
		}
...
func normalizeComparisonModes(manifest Manifest) Manifest {
	if len(manifest.Fixtures) == 0 {
		return manifest
	}
	manifest.Fixtures = append([]FixtureSpec(nil), manifest.Fixtures...)
	for i := range manifest.Fixtures {
		if manifest.Fixtures[i].ComparisonMode == "" {
			manifest.Fixtures[i].ComparisonMode = ComparisonSemantic
		}
	}
	return manifest
}
