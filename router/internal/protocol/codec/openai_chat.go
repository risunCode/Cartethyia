...
func encodeChatMessages(msgs []NormalizedMessage) []map[string]any {
	...
	for _, m := range msgs {
		...
		case RoleUser:
			hasStructured := false
			for _, b := range m.Content {
				if b.Type == BlockImage || b.Type == BlockNative || b.Type == BlockUnknown {
					hasStructured = true
					break
				}
			}
		...
		case RoleAssistant:
			...
			for _, b := range m.Content {
				if (b.Type == BlockNative || b.Type == BlockUnknown) && b.Raw != nil {
					if content, ok := entry["content"].(string); ok && content != "" {
						entry["content"] = []map[string]any{{"type": "text", "text": content}, cloneMap(b.Raw)}
					} else {
						entry["content"] = []map[string]any{cloneMap(b.Raw)}
					}
					break
				}
			}
