...
func (e *OpenAIChatResponseEncoder) Encode(ctx context.Context, response *NormalizedResponse) (map[string]any, *TransformError) {
	...
	if parts := encodeChatResponseMedia(response); len(parts) > 0 {
...
func encodeChatResponseMedia(response *NormalizedResponse) []map[string]any {
	if response == nil {
		return nil
	}
	...
	for _, block := range response.Output {
		if block.Type != BlockMediaOutput || block.Media == nil {
			continue
		}
		hasMedia = true
	}
	...
	if response.Text != "" {
		parts = append(parts, map[string]any{"type": "text", "text": response.Text})
	}
	for _, block := range response.Output {
		if block.Type != BlockMediaOutput || block.Media == nil {
			continue
		...
		parts = append(parts, map[string]any{
			"type": "image_url",
			"image_url": map[string]any{"url": responseMediaValue(block.Media)},
		})
	}
	return parts
}

func responseMediaValue(media *MediaReference) string {
	if media == nil {
		return ""
	}
	if media.Reference != ReferenceInlineData || len(media.Value) >= 5 && media.Value[:5] == "data:" {
		return media.Value
	}
	mime := media.MIMEType
	if mime == "" {
	...
	return "data:" + mime + ";base64," + media.Value
}
...
func (e *OpenAIChatResponseEncoder) EncodeEvent(ctx context.Context, event *NormalizedEvent) (map[string]any, *TransformError) {
	...
	case EventUsage:
		...
		return map[string]any{"object": "chat.completion.chunk", "choices": []any{}, "usage": encodeChatUsage(event.Usage)}, nil
