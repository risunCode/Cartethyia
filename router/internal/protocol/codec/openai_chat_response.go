...
func decodeOpenAIChatChoice(choice map[string]any, response *NormalizedResponse, index int) *TransformError {
	...
	if content := responseString(message["content"]); content != "" {
		...
		response.Output = append(response.Output, ContentBlock{Type: BlockText, Index: Value(indexValue), Text: content})
		...
		for partIndex, raw := range parts {
			...
			case "text":
				...
				response.Text += text
				response.Output = append(response.Output, ContentBlock{Type: BlockText, Index: Value(partIndex), Text: text, Annotations: decodeAnnotations(part["annotations"])})
			case "refusal":
				refusal := responseString(part["refusal"])
				response.Output = append(response.Output, ContentBlock{Type: BlockRefusal, Index: Value(partIndex), Refusal: &RefusalContent{Text: refusal}})
				response.Events = append(response.Events, NormalizedEvent{Type: EventRefusalDelta, Refusal: &RefusalContent{Text: refusal}, Index: Value(partIndex)})
			case "image_url":
				if image, ok := part["image_url"].(map[string]any); ok {
					value := responseString(image["url"])
					if value != "" {
						ref := classifyImageReference(value)
						reference := ReferenceURL
						if ref.Kind == ImageData {
							reference = ReferenceInlineData
						}
						media := &MediaReference{Media: MediaImage, Reference: reference, Value: ref.Value, MIMEType: ref.MediaType}
						response.Output = append(response.Output, ContentBlock{Type: BlockMediaOutput, Index: Value(partIndex), Media: media})
						response.Events = append(response.Events, NormalizedEvent{Type: EventItemDone, Media: media, Index: Value(partIndex)})
					}
				}
			}
		}
...
func decodeOpenAIToolCalls(raw []any, response *NormalizedResponse) *TransformError {
	for i, rawCall := range raw {
		...
		response.Output = append(response.Output, ContentBlock{Type: BlockToolUse, ID: tc.ItemID, ToolCallID: id, ToolName: name, ToolArguments: args, ToolKind: kind})
	}
	return nil
}
...
type OpenAIChatStreamDecoder struct {
	providerID string
	decoder    OpenAIChatResponseDecoder
}
...
func (d *OpenAIChatStreamDecoder) Decode(ctx context.Context, frame []byte) (*NormalizedEvent, error) {
	...
	event, terr := d.decoder.DecodeEvent(ctx, frame)
	if terr != nil {
		if d.providerID != "" {
			return nil, fmt.Errorf("%s: %w", d.providerID, terr)
		}
		return nil, terr
	}
	return event, nil
...
func decodeOpenAIChatChunk(chunk map[string]any) (*NormalizedEvent, *TransformError) {
	...
	if usage, ok := chunk["usage"].(map[string]any); ok {
		parsed := responseUsage(usage)
		event := &NormalizedEvent{Type: EventUsage, Usage: &parsed}
		if choice != nil {
			if finish := responseString(choice["finish_reason"]); finish != "" {
				reason := stopReasonFromString(finish)
	...
	if choice != nil {
		if finish := responseString(choice["finish_reason"]); finish != "" {
			reason := stopReasonFromString(finish)
			return &NormalizedEvent{Type: EventResponseCompleted, StopReason: &reason}, nil
		}
	}
	return nil, nil
}
