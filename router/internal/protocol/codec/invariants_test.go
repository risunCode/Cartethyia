...
func TestNormalizeToolCallInvariantsCompactsArgumentsOnceAndPreservesIDs(t *testing.T) {
	...
	if call.ToolCallID != id || call.ToolArguments != `{"q":"x"}` {
		t.Fatalf("call identity/arguments changed: %#v", call)
	}
	if report.NormalizedArguments != 1 || report.DroppedCalls != 0 {
		t.Fatalf("unexpected report: %#v", report)
	}
	if len(out.Messages) != 2 || out.Messages[1].Content[0].ToolCallID != id {
		t.Fatalf("adjacency lost: %#v", out.Messages)
	}
	if req.Messages[0].Content[0].ToolArguments != " { \"q\": \"x\" } " {
...
func TestNormalizeToolCallInvariantsDropsOrphansAndUnansweredCalls(t *testing.T) {
	...
	if report.DroppedCalls != 1 || report.DroppedResults < 1 {
		t.Fatalf("unexpected report: %#v", report)
	}
	if len(out.Messages) != 3 || out.Messages[0].Role != RoleAssistant || out.Messages[1].Role != RoleTool || out.Messages[2].Role != RoleUser {
		t.Fatalf("adjacency = %#v", out.Messages)
	}
	if len(out.Messages[0].Content) != 1 || out.Messages[0].Content[0].ToolCallID != "call_answered" || out.Messages[1].Content[0].ToolCallID != "call_answered" {
		t.Fatalf("wrong pair = %#v", out.Messages)
	}
}
...
func TestNormalizeToolCallInvariantsReordersResultsToCallOrder(t *testing.T) {
	...
	if report.ReorderedResults == 0 || len(out.Messages) != 3 {
		t.Fatalf("expected reorder: %#v", report)
	}
	if out.Messages[1].Content[0].ToolCallID != "a" || out.Messages[2].Content[0].ToolCallID != "b" {
		t.Fatalf("order = %#v", out.Messages)
	}
}
...
func TestNormalizeToolCallInvariantsRejectsMalformedArguments(t *testing.T) {
	...
	if _, _, err := NormalizeToolCallInvariants(req, ToolCallInvariantDrop); err == nil {
...
func TestNormalizeToolCallInvariantsGeneratesUniqueMissingIDs(t *testing.T) {
	...
	if report.GeneratedCallIDs != 2 || calls[0].ToolCallID == calls[1].ToolCallID || calls[0].ToolCallID == calls[2].ToolCallID || calls[1].ToolCallID == calls[2].ToolCallID {
		t.Fatalf("generated IDs collided: report=%#v calls=%#v", report, calls)
	}
}
