...
func TestGeminiNativeGoldenRoundTripPreservesPartOrder(t *testing.T) {
	...
	if len(request.Messages) != 4 || request.Messages[2].Content[0].Type != BlockToolUse || request.Messages[3].Content[0].Type != BlockToolResult {
	...
	if got := decoded.Messages[0].Content[0].Text; got != "system" {
		t.Fatalf("system text=%q", got)
	}
	if got := decoded.Messages[1].Content[1].Type; got != BlockImage {
		t.Fatalf("media type=%q", got)
	}
	if got := decoded.Messages[2].Content[0].ToolCallID; got != "call-1" {
...
func TestGeminiRegistryIncludesNativeIngress(t *testing.T) {
	...
	if !ok || decoder == nil {
		t.Fatal("Gemini decoder not registered")
	}
	encoder, ok := registry.LookupRequest(contracts.ProtocolGemini)
	if !ok || encoder == nil {
		t.Fatal("Gemini encoder not registered")
	}
}
...
func TestCompactionCodecV1AndV2Bridge(t *testing.T) {
	...
	if encoded.Wire["compact"] != true {
		t.Fatalf("v1 wire=%#v", encoded.Wire)
	}
	bridged, bridgeErr := BridgeCompactionRequest(request, CompactionV2)
	if bridgeErr != nil {
		t.Fatal(bridgeErr)
	}
	if bridged.Version != CompactionV2 || bridged.Input[len(bridged.Input)-1].Type != BlockCompactionTrigger {
		t.Fatalf("bridge=%#v", bridged)
	}
}
