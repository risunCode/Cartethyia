-- ============================================================================
-- 0001_initial.sql — initial schema for Cartethyia dashboard persistence
--
-- Tables:
--   users          account identities (id, email, display name, timestamps)
--   user_settings  per-user dashboard preferences (key/value JSON)
--   api_keys       server-tracked API tokens (hashed) for the dashboard
--   quota_accounts per-provider quota snapshots
--   share_links    shareable URL tokens mapping to payloads
--
-- All tables include `created_at` / `updated_at` for audit; indices are added
-- to support the dashboard's hot read paths.
-- ============================================================================
