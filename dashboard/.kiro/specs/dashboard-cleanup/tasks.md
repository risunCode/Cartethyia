...
- [x] 1. Establish route and entrypoint characterization tests
  ...
  - Add regression tests proving retired `/customization` and `/token-saver` paths are not registered.
...
- [x] 3. Introduce shared deterministic test helpers
...
- [x] 4. Strengthen model-picker and provider contract tests
  - [x] 4.1 Cover picker interaction and catalog boundaries
  ...
  - [x] 4.2 Extract and test provider normalizers/builders
...
- [x] 5. Add high-risk lifecycle and destructive-action tests
  - [x] 5.1 Test async lifecycle cleanup
    - Cover EventSource open/error/reconnect/unmount behavior and the extracted Model Studio SSE transport.
    - Keep stream abort behavior explicit through the transport signal contract.
    - _Requirements: 3.3, 5.2_
  - [x] 5.2 Test destructive and protected flows
    - Cover settings backup validation, password confirmation, destructive API requests, mutation failure, and query invalidation boundaries.
    - Cover safe same-origin login destinations and unauthorized API handling.
    - _Requirements: 3.1, 3.2, 3.4, 5.2_

- [x] 6. Extract only stable boundaries from hotspot files
  - [x] 6.1 Extract shell concerns from `src/app/layout.tsx`
  ...
  - [x] 6.2 Extract model-studio boundaries
    - Separate stream transport and usage normalization without changing abort, attachment, token, or autosave semantics.
    - Keep `ModelStudioPage` as the orchestration entrypoint.
    - _Requirements: 2.1, 2.3, 3.3_
  - [x] 6.3 Extract stable provider detail boundaries
    - Separate provider normalization and model-test/error helpers; keep stateful account/OAuth/routing mutation ownership together.
    - Preserve existing public callbacks and focused provider contracts.
    - _Requirements: 2.1, 2.3, 3.2_

- [x] 7. Investigate and remove cleanup debt at the source
  - Fix the existing `backdrop-blur-[6px]` CSS build warning without suppressing it.
  - Remove obsolete wrappers/imports and dead aliases only after references and tests prove they are unused.
  - Keep the Automation placeholder and `Soon` badge unchanged.
  - _Requirements: 4.1, 4.3, 4.4, 5.3_

- [x] 8. Verify each cleanup batch end to end
