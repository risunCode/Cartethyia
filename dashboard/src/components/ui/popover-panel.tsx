...
export function PopoverPanel(props: { className?: string; children: JSX.Element }): JSX.Element {
  return <div class={cn("z-50 max-h-[min(70vh,28rem)] overflow-y-auto rounded-2xl border border-[var(--inner-border)] bg-[var(--glass-bg-2)] p-3 text-[var(--text-1)] shadow-2xl backdrop-blur-[6px]", props.className)}>{props.children}</div>;
}
