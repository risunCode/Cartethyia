  ...
  return (
    <div class={`story story-simple story-theme-${CHAPTERS[activeIndex()]?.theme ?? "night"}`} id="top">
      <header class="story-header">
        ...
        <div class={`story-mobile-overlay ${menuOpen() ? "is-open" : ""}`} aria-hidden={true} onClick={() => setMenuOpen(false)} />
        <nav class={`story-mobile-nav ${menuOpen() ? "is-open" : ""}`} aria-label="Mobile navigation" aria-hidden={!menuOpen()}>
          <div class="story-mobile-nav-head">
            <span>Navigation</span>
            <button type="button" aria-label="Close menu" onClick={() => setMenuOpen(false)}>
              <X size={17} aria-hidden={true} />
            </button>
          </div>
          <For each={CHAPTERS}>{(entry, index) => <button type="button" onClick={() => scrollToChapter(index())}><span>{entry.number}</span>{entry.label}<ArrowUpRight size={13} aria-hidden={true} /></button>}</For>
          <a href={CONSOLE_LOGIN_PATH}>Enter console<ArrowUpRight size={14} aria-hidden={true} /></a>
        </nav>
      </header>
      ...
      <main class="story-simple-main">
        <For each={CHAPTERS}>
          {(entry, index) => (
            <article id={`chapter-${index()}`} data-chapter-index={index()} ref={(node) => { sectionRefs[index()] = node; }} class={`story-page-section story-theme-${entry.theme} ${entry.sectionClass ?? ""} ${revealedChapters().has(index()) ? "is-revealed" : ""}`} aria-labelledby={`chapter-title-${index()}`}>
