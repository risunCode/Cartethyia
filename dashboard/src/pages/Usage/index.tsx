      ...
      >
        <div class="h-56 w-full" role="img" aria-label="Requests per bucket">
          <svg viewBox="0 0 800 240" class="h-full w-full" preserveAspectRatio="none">
            ...
            <For each={[30, 75, 120, 165, 210]}>{(y) => <line x1="20" x2="780" y1={y} y2={y} stroke="var(--inner-border)" stroke-dasharray="3 3" opacity="0.6" />}</For>
            <polygon points={area()} fill="url(#usageChartFill)" class="transition-opacity duration-300" />
            <polyline points={line()} fill="none" stroke="var(--accent)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" vector-effect="non-scaling-stroke" class="transition-all duration-300" />
            <For each={linePoints()}>
              {(point) => (
                        <circle cx={point.x} cy={point.y} r="4" fill="var(--surface-1)" stroke="var(--accent)" stroke-width="2" class="cursor-pointer opacity-0 transition-opacity duration-150 hover:opacity-100">
    ...
    <div class="dashboard-page animate-fade-in space-y-5">
      ...
      <section aria-label="Summary metrics">
        <Show
          when={!isLoading()}
          fallback={
            <div class="grid grid-cols-2 gap-3 card-stagger md:grid-cols-4">
