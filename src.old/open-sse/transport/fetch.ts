...
/**
 * Executes a real fetch bound to the coordinator signal. Pre-response
 * transport timeouts surface as network_unavailable (routeScope "proxy");
 * caller aborts as client_aborted; transport failures as network_unavailable.
 */
...
export async function executeFetch(url: string, init: RequestInit, coordinator: AbortCoordinator, network?: NetworkSelection, capture?: { request(value: unknown): void; observeResponse(response: Response): Response }): Promise<Response> {
  try {
    ...
    coordinator.markResponseHeadersReceived();
    ...
    if (isAbortError(error)) {
      ...
      const message = coordinator.causeOf() === "first_byte_timeout"
        ? "Upstream first-byte timeout"
        : "Upstream connection timed out";
      throw new ProviderAdapterError({ kind: "network_unavailable", message, retryable: true, routeScope: "proxy" });
    }
