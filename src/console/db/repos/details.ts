 ...
 * console_logs instead of an in-process TTL/cap. Inserts go through
 * `../runtime-write-buffer.ts` (batched commits); reads flush that buffer
 * first via `readRuntimeDb()`.
 */
...
export function insertAssetMeta(input: AssetMetaInsert): void {
  enqueueRuntimeWrite(
    "INSERT INTO request_assets (request_id, kind, mime, bytes, sha256, storage_path, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    [input.requestId, input.kind, input.mime, input.bytes, input.sha256, input.storagePath, new Date().toISOString()],
  );
}
...
export function insertToolCall(input: ToolCallInsert): void {
  enqueueRuntimeWrite(
    "INSERT INTO request_tool_calls (request_id, name, bytes, sha256, duration_ms, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
    [input.requestId, input.name, input.bytes, input.sha256, input.durationMs, input.status, new Date().toISOString()],
  );
}
...
export function deleteRequestDetailsOlderThan(cutoffDate: string): number {
  return readRuntimeDb().query("DELETE FROM request_details WHERE created_at < ?").run(cutoffDate).changes;
}
...
export function deleteRequestToolCallsOlderThan(cutoffDate: string): number {
  return readRuntimeDb().query("DELETE FROM request_tool_calls WHERE created_at < ?").run(cutoffDate).changes;
}
