...
export function runtimeSettings(config: ConfigPersistence, env: PersistenceEnv = config.env): ConsoleRuntimeSettings {
  ...
  return {
    ...
    trustProxy: value.trustProxy === true || Bun.env.TRUST_PROXY === "true",
    cacheMarkersEnabled: true,
    sessionTtlHours: typeof value.sessionTtlHours === "number" ? Math.min(168, Math.max(1, Math.floor(value.sessionTtlHours))) : 12,
