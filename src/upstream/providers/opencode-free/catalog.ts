...
export async function fetchOpenCodeFreeCatalog(): Promise<OpenCodeModelEntry[]> {
  const now = Date.now();
...
async function fetchCatalog(): Promise<OpenCodeModelEntry[]> {
  ...
  if (!res.ok) {
    cache = undefined;
    throw new ProviderCallError(503, "unavailable", "Could not fetch the OpenCode Free model catalog.");
  }

  const body: unknown = await res.json();
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    cache = undefined;
    throw new ProviderCallError(502, "malformed_response", "OpenCode Free catalog returned an unexpected shape.");
  }

  const data = (body as Record<string, unknown>).data;
  if (!Array.isArray(data)) {
    cache = undefined;
    throw new ProviderCallError(502, "malformed_response", "OpenCode Free catalog is missing the model list.");
  }
...
export function resetOpenCodeFreeCatalogForTests(): void {
  cache = undefined;
}