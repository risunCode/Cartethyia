...
function parseCodexQuota(body: unknown, fetchedAt: string): ProviderQuotaResult {
  ...
  const add = (raw: unknown, fallback: string): void => {
    ...
    const reset =
      isoDate(value.reset_at) ??
      (afterSeconds !== null
        ? new Date(Date.parse(fetchedAt) + afterSeconds * 1000).toISOString()
        : null);
    if (used !== null || reset !== null) {
      const kind =
        duration !== null && duration <= 21_600
          ? "session"
          : duration !== null && duration <= 86_400
            ? "daily"
            : "weekly";
      const label =
        duration !== null ? `${Math.max(1, Math.round(duration / 3600))} Hour` : fallback;
...
export async function fetchCodexQuota(
  ...
  return parseCodexQuota(body, new Date().toISOString());
}
