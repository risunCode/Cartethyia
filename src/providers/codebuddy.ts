...
].map(([id, name, reasoning, vision, contextWindow, maxOutputTokens]) => modelOf(id as string, name as string, capabilitiesOf({ surfaces: SURFACES, reasoning: reasoning as boolean, images: vision as boolean }), { context: { inputTokens: contextWindow as number | null, outputTokens: maxOutputTokens as number | null } }));

const chinaModels: readonly ProviderModel[] = [
  ["glm-5.2", "GLM 5.2", false, true, 1_000_000, 8_192], ["glm-5.1", "GLM 5.1", false, true, 200_000, 8_192], ["glm-5.0", "GLM 5.0", false, true, 200_000, 8_192], ["glm-5.0-turbo", "GLM 5.0 Turbo", false, true, 200_000, 8_192], ["glm-5v-turbo", "GLM 5v Turbo", false, true, 200_000, 8_192], ["glm-4.7", "GLM 4.7", false, true, 200_000, 8_192], ["minimax-m3", "MiniMax M3", false, true, 512_000, 8_192], ["minimax-m2.7", "MiniMax M2.7", false, true, 512_000, 8_192], ["kimi-k2.7", "Kimi K2.7", true, true, 256_000, 8_192], ["kimi-k2.6", "Kimi K2.6", false, true, 256_000, 8_192], ["kimi-k2.5", "Kimi K2.5", false, true, 164_000, 8_192], ["hy3-preview", "Hunyuan Hy3 Preview", false, false, 192_000, 8_192], ["deepseek-v4-pro", "DeepSeek V4 Pro", false, true, 1_000_000, 8_192], ["deepseek-v4-flash", "DeepSeek V4 Flash", false, true, 1_000_000, 8_192], ["deepseek-v3-2-volc", "DeepSeek V3.2", false, true, 64_000, 8_192],
].map(([id, name, reasoning, vision, contextWindow, maxOutputTokens]) => modelOf(id as string, name as string, capabilitiesOf({ surfaces: SURFACES, reasoning: reasoning as boolean, images: vision as boolean }), { context: { inputTokens: contextWindow as number | null, outputTokens: maxOutputTokens as number | null } }));

function actualModel(id: string, providerId: string): string {
...
abstract class CodeBuddyBaseAdapter implements ProviderAdapter {
  ...
  async call(input: ProviderRequest): Promise<ProviderOutput> {
    ...
    const target = { ...input.target, modelId: actualModel(input.target.modelId, this.metadata.id) };
    return callChatCompletionsWire({ ...input, target, request: { ...input.request, model: target.modelId } }, this.baseUrl, headers(input.credential, this.metadata.id === "codebuddy" ? "www.codebuddy.ai" : "www.codebuddy.cn", input.headers));
  }
