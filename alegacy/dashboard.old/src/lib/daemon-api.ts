import { api, ApiError, sanitizeErrorMessage, type DaemonHttpMethod } from "./api";
import { isDocumentedDaemonRoute } from "./daemon-routes";

/** Envelope emitted by the daemon's /v2/admin routes. */
...
/** Removes secret-shaped fields before an unknown daemon value can enter UI state. */
...
/** Normalizes a successful daemon envelope and rejects malformed contracts. */
export function unwrapDaemonEnvelope<T>(value: unknown): T {
  if (!isRecord(value) || !("data" in value)) {
    throw new DaemonContractError("invalid_contract", "daemon response envelope is invalid", 502);
  }
...
/** Execute a typed daemon admin request through the same-origin V2 API client. */
export async function daemonApi<T>(route: string, init: RequestInit = {}): Promise<T> {
  const normalized = route.startsWith("/") ? route : `/${route}`;
  const method = (init.method ?? "GET").toUpperCase() as DaemonHttpMethod;
  if (normalized.startsWith("/v1/") || normalized.startsWith("/v2/") || normalized.includes("://") || !isDocumentedDaemonRoute(normalized, method)) {
    throw new DaemonContractError("invalid_route", "dashboard route is outside the retained V2 admin contract", 400);
  }
  const response = await api<DaemonEnvelope<unknown>>(`${ADMIN_PREFIX}${normalized}`, init);
  return unwrapDaemonEnvelope<T>(response);
}

export const daemonGet = <T>(route: string) => daemonApi<T>(route, { method: "GET" });
export const daemonPost = <T>(route: string, body?: unknown) => daemonApi<T>(route, { method: "POST", body: JSON.stringify(body ?? {}) });
export const daemonPatch = <T>(route: string, body?: unknown) => daemonApi<T>(route, { method: "PATCH", body: JSON.stringify(body ?? {}) });
export const daemonDelete = <T>(route: string) => daemonApi<T>(route, { method: "DELETE" });

/** Converts daemon failures into bounded state suitable for an operator view. */
export function daemonFailure(error: unknown): { code: string; message: string; degraded: boolean } {
  if (error instanceof ApiError || error instanceof DaemonContractError) {
    return {
      code: error.code,
...
/** Parses the redacted dashboard summary contract without retaining arbitrary metadata. */
export function normalizeDashboardSummary(value: unknown): DashboardSummary {
  if (!isRecord(value)) throw new DaemonContractError("invalid_contract", "dashboard summary is invalid", 502);
...
/** Parses telemetry overview while preserving unavailable values as null. */
export function normalizeTelemetryOverview(value: unknown): TelemetryOverview {
  if (!isRecord(value)) throw new DaemonContractError("invalid_contract", "telemetry overview is invalid", 502);
...
/** Parses a bounded telemetry bucket list returned by the daemon. */
export function normalizeTelemetryBuckets(value: unknown): TelemetryBucket[] {
  if (!isRecord(value) || !Array.isArray(value.items)) throw new DaemonContractError("invalid_contract", "telemetry buckets are invalid", 502);
...
/** Catalog routes are optional until the daemon advertises a catalog service. */
export async function daemonCatalog(): Promise<readonly CatalogProvider[]> {
  const value = await daemonGet<unknown>("/catalog/providers");
  if (!isRecord(value) || !Array.isArray(value.items)) throw new DaemonContractError("invalid_contract", "catalog response is invalid", 502);
