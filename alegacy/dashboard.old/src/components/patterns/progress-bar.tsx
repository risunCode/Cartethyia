...
export function ProgressBar({ value, max = 100, tone = "accent", label, showValue = false, className }: ProgressBarProps) {
  ...
  return (
    <div className={cn("min-w-0", className)}>
      {(label || showValue) && (
        <div className="mb-1 flex items-center justify-between gap-2 text-[10px] text-[var(--text-3)]">
          {label && <span className="truncate">{label}</span>}
          {showValue && <span className="tabular-nums">{Math.round(percentage)}%</span>}
        </div>
      )}
      <div className="h-1.5 overflow-hidden rounded-full bg-[var(--track)]" role="progressbar" aria-label={label} aria-valuemin={0} aria-valuemax={normalizedMax} aria-valuenow={normalizedValue}>
        <div className={cn("h-full origin-left rounded-full transition-transform duration-500", toneClasses[tone])} style={{ transform: `scaleX(${percentage / 100})` }} />
      </div>
    </div>
  );
}
