...
Catatan penting: dashboard aux **wajib** DB terpisah — daemon dan dashboard sama-sama punya tabel bernama `api_keys` & `share_links` dengan schema berbeda (lihat §3).
...
erDiagram
    ...
    users ||--o{ user_settings : "cascade"
    users ||--o{ api_keys : "cascade"
    users ||--o{ quota_accounts : "cascade"
    users ||--o{ share_links : "set null"
```
...
**⚠️ Collision yang harus selalu diingat**: `api_keys` & `share_links` ada di KEDUA database dengan bentuk beda total (daemon: TEXT id + limit token; dashboard: UUID + FK users). Inilah alasan aux wajib DB terpisah — detail di `v2.1-execution-report.md` §7.
...
| # | Temuan | Lokasi | Dampak |
|---|---|---|---|
| B1 | **Schema aux dashboard orphan total** — satu-satunya konsumen pool adalah `runMigrations`; tidak ada query ke `users`/`user_settings`/`api_keys`/`quota_accounts`/`share_links` di seluruh dashboard/src | `dashboard/src/server/index.ts:22`; `dashboard/migrations/0001_initial.sql` | 5 tabel hidup tanpa pembaca/penulis — migration mengada tanpa manfaat |
