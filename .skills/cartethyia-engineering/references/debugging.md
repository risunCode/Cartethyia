...
D. Capability note: `image` falls open for every non-`native` wire in `buildCapabilityProfile` — a codec-backed route carries the part regardless of the row's declared modalities, and the upstream decides whether it accepts it. Only a `native` adapter (Cursor, Devin) needs an explicit `image` modality.
...
3. `bun run db:migrate` on each; verify `cartethyia_schema_migrations` holds `0000_baseline.sql`. Start the backend once on the fresh main DB (production seeding runs at boot), expect `/health/ready` 200, and sane provider/model counts. Stop the smoke backend.
