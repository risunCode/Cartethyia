...
4. Dashboard same change: `dashboard/src/lib/contracts.ts` (re-export, no second type source) → `assertProviderRouting` in `dashboard/src/lib/hooks/common.ts` → `dashboard/src/lib/hooks/routing.ts` (`useProviderRouting`, `useUpdateProviderRouting`) and `dashboard/src/lib/use-routing-strategy.ts` (`useRoutingStrategy`, saves via `dashboard/src/lib/use-debounced-save.ts`) → `dashboard/src/routes/provider-detail/RoutingStrategyCard.tsx` (`RoutingStrategyCard`, parity with `dashboard/src/routes/Proxy.tsx`). Backend `updateRouting` calls `await snapshotInvalidator?.invalidate()` so the next `/v1/*` request picks up operator edits; new static columns still need the numbered migration applied at boot (restart the process to run it).
