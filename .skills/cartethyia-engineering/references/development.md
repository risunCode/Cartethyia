...
3. Schema pins: `test/contracts/migration-integrity.contract.test.ts` pins `network_pools` columns/indexes/checks; `backup_status` is pinned by the baseline plus `test/integration/isolated-db.test.ts` — update migration assertions and schema together.
