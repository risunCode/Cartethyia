...
type fakeProxyAdmin struct {
	...
	deleteErr   error
	created     []ProxyInput
	updated     map[string]ProxyInput
	deleted     []string
	updateCalls int
	deleteCalls int
}

func (f *fakeProxyAdmin) List(context.Context) ([]ProxyRecord, error) {
	if f.listErr != nil {
	...
	return append([]ProxyRecord(nil), f.list...), nil
}

func (f *fakeProxyAdmin) Create(_ context.Context, input ProxyInput) (ProxyRecord, error) {
	if f.createErr != nil {
		return ProxyRecord{}, f.createErr
	}
...
func (f *fakeProxyAdmin) Update(_ context.Context, id string, input ProxyInput) (ProxyRecord, error) {
	f.updateCalls++
	if f.updated == nil {
		f.updated = map[string]ProxyInput{}
	}
	f.updated[id] = input
	if f.updateErr != nil {
		return ProxyRecord{}, f.updateErr
	}
	return ProxyRecord{ID: id, Host: derefString(input.Host), Port: derefInt(input.Port)}, nil
}
...
func TestProxyHandlersListReturnsSeededEntries(t *testing.T) {
	svc := &fakeProxyAdmin{list: []ProxyRecord{{ID: "p1", Host: "10.0.0.1", Port: 8080}}}
