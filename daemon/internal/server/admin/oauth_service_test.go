...
func TestOAuthServiceBrowserCompletionPersistsRedactedAccount(t *testing.T) {
	...
	registry, err := drivers.NewRegistry(map[string]drivers.Config{"codex": {HTTPClient: &http.Client{Transport: transport}}})
	if err != nil {
		t.Fatal(err)
	}
	...
	service, err := NewOAuthService(registry, flow.NewManager(flow.ManagerOptions{}), accountsStore, secrets, records, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOAuthServiceKiroAWSDevicePendingThenCompleted(t *testing.T) {
	...
	registry, err := drivers.NewRegistry(map[string]drivers.Config{"kiro": {HTTPClient: &http.Client{Transport: transport}}})
	if err != nil {
		t.Fatal(err)
	}
	service, err := NewOAuthService(registry, flow.NewManager(flow.ManagerOptions{TTL: time.Minute}), accounts.NewMemoryAccountConfigStore(), accounts.NewMemorySecretStore(), accounts.NewMemoryRecordStore(), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOAuthServiceKiroManualJSONImportIsRedacted(t *testing.T) {
	service, err := NewOAuthService(mustRegistry(t), flow.NewManager(flow.ManagerOptions{}), accounts.NewMemoryAccountConfigStore(), accounts.NewMemorySecretStore(), accounts.NewMemoryRecordStore(), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOAuthServiceRefreshReturnsRedactedAccountState(t *testing.T) {
	...
	service, err := NewOAuthService(mustRegistry(t), flow.NewManager(flow.ManagerOptions{}), accounts.NewMemoryAccountConfigStore(), accounts.NewMemorySecretStore(), accounts.NewMemoryRecordStore(), refresher)
	if err != nil {
		t.Fatal(err)
	}
...
func mustRegistry(t *testing.T) *drivers.Registry {
	t.Helper()
	registry, err := drivers.NewRegistry(nil)
	if err != nil {
		t.Fatal(err)
	}
	return registry
