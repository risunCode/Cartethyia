...
func (*authAuthorityStub) Reserve(context.Context, tokenbudget.ReservationRequest) (tokenbudget.TokenReservation, error) {
	return nil, nil
}
...
func TestPublicV1AuthTransfersOnlyDurableQuotaAuthorityAndRedactedIdentity(t *testing.T) {
	...
	handler := RequestID(PublicV1Auth(authResolverStub{key: PublicAPIKey{
		...
		gotAuthority, identity, ok := tokenbudget.AuthorityFromContext(r.Context())
		if !ok {
