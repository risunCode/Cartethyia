...
func registerModels(mux *http.ServeMux, deps Deps) {
	mux.HandleFunc(ModelsPath, func(w http.ResponseWriter, r *http.Request) {
		handleModels(w, r, deps.Catalog)
	})
}

func handleModels(w http.ResponseWriter, r *http.Request, catalog ModelCatalog) {
	if r.Method != http.MethodGet {
		MethodNotAllowed(w, http.MethodGet)
		return
	}
	if catalog == nil {
		Write(w, http.StatusServiceUnavailable, CodeInternal, "model catalog is not configured")
		return
	}
