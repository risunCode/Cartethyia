...
func handleCountTokens(w http.ResponseWriter, r *http.Request) {
	...
	if !wire.HasJSONContentType(r) {
		apierrors.Write(w, http.StatusUnsupportedMediaType, apierrors.CodeUnsupportedMedia, "count_tokens requires Content-Type: application/json")
		return
	}
	...
	if transformErr != nil {
		apierrors.Write(w, http.StatusBadRequest, apierrors.CodeInvalidRequest, "count_tokens request is invalid")
		return
	}
	count, ok := estimateInputTokens(decoded)
	if !ok {
		apierrors.Write(w, http.StatusBadRequest, apierrors.CodeInvalidRequest, "count_tokens request exceeds the counting bound")
		return
	}
