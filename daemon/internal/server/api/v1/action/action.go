...
type streamEvidence interface {
	RequestMetadata() apicontracts.RequestMetadata
}

func Register(mux *http.ServeMux, deps Deps) {
	mux.HandleFunc(Path, func(w http.ResponseWriter, r *http.Request) { handle(w, r, deps.Proxy, deps.Evidence) })
}

func handle(w http.ResponseWriter, r *http.Request, proxy apicontracts.ProxyService, evidence *observability.Registry) {
	if r.Method != http.MethodPost {
		apierrors.MethodNotAllowed(w, http.MethodPost)
		return
	}
	if proxy == nil {
		apierrors.Write(w, http.StatusServiceUnavailable, apierrors.CodeInternal, "action proxy is not configured")
		return
	}
	if !wire.HasJSONContentType(r) {
		apierrors.Write(w, http.StatusUnsupportedMediaType, apierrors.CodeUnsupportedMedia, "action requires Content-Type: application/json")
		return
	}
	...
	if err := json.Unmarshal(body, &envelope); err != nil {
		apierrors.Write(w, http.StatusBadRequest, apierrors.CodeInvalidRequest, "action body must be a JSON object")
		return
	}
	protocol := contracts.Surface(envelope.Protocol)
	if !protocol.IsValid() || protocol == contracts.SurfaceWebSearch {
		apierrors.Write(w, http.StatusBadRequest, apierrors.CodeInvalidRequest, "action protocol is unsupported")
		return
	}
...
func applyRequestMetadata(event *observability.RequestEvent, metadata apicontracts.RequestMetadata) {
	if event == nil {
		return
	}
