...
func TestProxyAdminSQLMockCRUD(t *testing.T) {
	...
	t.Run("create success", func(t *testing.T) {
		...
		host := "127.0.0.1"
		port := 8080
		got, err := NewBunProxyAdminRepository(db).Create(ctx, admin.ProxyInput{
	...
	t.Run("create validation rejects missing fields", func(t *testing.T) {
		...
		host := "127.0.0.1"
		port := 8080
		if _, err := NewBunProxyAdminRepository(db).Create(ctx, admin.ProxyInput{Host: &host, Port: &port}); err == nil {
	...
	t.Run("create validation rejects unsupported type", func(t *testing.T) {
		...
		host := "127.0.0.1"
		port := 8080
		if _, err := NewBunProxyAdminRepository(db).Create(ctx, admin.ProxyInput{Type: &bad, Host: &host, Port: &port}); err == nil {
			t.Fatal("expected validation error")
		}
	})
	...
	t.Run("create validation rejects invalid port", func(t *testing.T) {
		...
		host := "127.0.0.1"
		port := 0
		if _, err := NewBunProxyAdminRepository(db).Create(ctx, admin.ProxyInput{Type: &kind, Host: &host, Port: &port}); err == nil {
			t.Fatal("expected validation error")
		}
	})
	...
	t.Run("update success", func(t *testing.T) {
		...
		expectAnyQueryRows(mock, healthCols, []any{"proxy-1", "healthy"})
		host := "10.0.0.1"
		got, err := NewBunProxyAdminRepository(db).Update(ctx, "proxy-1", admin.ProxyInput{Host: &host})
		if err != nil {
	...
	t.Run("update validation rejects unsupported type", func(t *testing.T) {
		db, _ := newFakeBun(t)
		bad := "ftp"
		if _, err := NewBunProxyAdminRepository(db).Update(ctx, "proxy-1", admin.ProxyInput{Type: &bad}); err == nil {
			t.Fatal("expected validation error")
		}
	})
	...
	t.Run("update validation rejects empty payload", func(t *testing.T) {
		db, _ := newFakeBun(t)
		if _, err := NewBunProxyAdminRepository(db).Update(ctx, "proxy-1", admin.ProxyInput{}); err == nil {
	...
	t.Run("update empty id rejected", func(t *testing.T) {
		db, _ := newFakeBun(t)
		host := "10.0.0.1"
		if _, err := NewBunProxyAdminRepository(db).Update(ctx, "  ", admin.ProxyInput{Host: &host}); err == nil {
	...
	t.Run("update not found", func(t *testing.T) {
		...
		host := "10.0.0.1"
		_, err := NewBunProxyAdminRepository(db).Update(ctx, "missing", admin.ProxyInput{Host: &host})
	...
	t.Run("update error", func(t *testing.T) {
		...
		host := "10.0.0.1"
		if _, err := NewBunProxyAdminRepository(db).Update(ctx, "proxy-1", admin.ProxyInput{Host: &host}); err == nil {
	...
	t.Run("closed repository", func(t *testing.T) {
		...
		host := "127.0.0.1"
		port := 8080
		if _, err := r.Create(ctx, admin.ProxyInput{Type: &kind, Host: &host, Port: &port}); err == nil {
			t.Fatal("expected closed Create error")
		}
		if _, err := r.Update(ctx, "x", admin.ProxyInput{Host: &host}); err == nil {
