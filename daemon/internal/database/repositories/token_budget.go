...
func (r *BunTokenBudgetRepository) Reserve(ctx context.Context, request tokenbudget.ReservationRequest) (tokenbudget.TokenReservation, error) {
	if err := request.Validate(); err != nil {
		return nil, err
	}
	if r == nil || r.db == nil {
		return nil, tokenbudget.ErrUnavailable
	}
	if ctx == nil {
	...
	err := r.db.RunInTx(ctx, &sql.TxOptions{}, func(ctx context.Context, tx bun.Tx) error {
		...
		if found {
			if existing.estimate != request.Estimate || !sameUTCDay(existing.dailyStart, dailyStart) || !sameUTCDay(existing.monthlyStart, monthlyStart) {
				return tokenbudget.ErrConflict
			}
			return nil
		}
...
func (r *durableReservation) Reconcile(ctx context.Context, tokens usage.Tokens) error {
	if r == nil || r.repository == nil || r.repository.db == nil {
		return tokenbudget.ErrUnavailable
	}
	actual, err := tokenbudget.AccountedTokens(tokens)
	if err != nil {
		return err
	}
	if ctx == nil {
...
func (r *durableReservation) Release(ctx context.Context, reason tokenbudget.ReleaseReason) error {
...
func (r *BunTokenBudgetRepository) release(ctx context.Context, keyID, requestID string, attempt int, reason tokenbudget.ReleaseReason) (bool, error) {
	released := false
	err := r.db.RunInTx(ctx, &sql.TxOptions{}, func(ctx context.Context, tx bun.Tx) error {
...
func (r *BunTokenBudgetRepository) RecoverExpired(ctx context.Context, now time.Time, limit int) (int, error) {
	if r == nil || r.db == nil {
		return 0, tokenbudget.ErrUnavailable
	}
	if ctx == nil {
		ctx = context.Background()
	}
...
func lockTokenLimits(ctx context.Context, tx bun.Tx, keyID string, requireActive bool) (tokenLimitRow, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return tokenLimitRow{}, tokenbudget.ErrUnavailable
	}
	return row, err
}
...
func translateTokenBudgetError(op string, err error) error {
	...
	for _, known := range []error{tokenbudget.ErrInvalid, tokenbudget.ErrLimit, tokenbudget.ErrConflict, tokenbudget.ErrUnavailable} {
	...
	return fmt.Errorf("%w: %s persistence failed", tokenbudget.ErrUnavailable, op)
}

var _ tokenbudget.TokenBudgetAuthority = (*BunTokenBudgetRepository)(nil)
var _ tokenbudget.ExpiredReservationRecoverer = (*BunTokenBudgetRepository)(nil)
