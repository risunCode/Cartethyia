...
func TestReadyTelemetryRepository(t *testing.T) {
	...
	mustClosed(t, (*MetadataSinkAdapter)(nil).WriteMetadata(ctx, observability.Metadata{}))
	mustClosed(t, (&MetadataSinkAdapter{}).WriteMetadata(ctx, observability.Metadata{}))
}
...
func TestReadyTokenBudgetRepository(t *testing.T) {
	ctx := context.Background()
	req := tokenbudget.ReservationRequest{
	...
	for _, r := range []*BunTokenBudgetRepository{nilR, empty} {
		...
		if !errors.Is(err, tokenbudget.ErrUnavailable) {
	...
	}{}); !errors.Is(err, tokenbudget.ErrUnavailable) {
...
func TestReadyTokenBudgetRepositoryHandles(t *testing.T) {
	...
	if err := nilHandle.Reconcile(ctx, usage.Tokens{}); !errors.Is(err, tokenbudget.ErrUnavailable) {
		t.Fatalf("nil Reconcile = %v", err)
	}
	if err := (&durableReservation{}).Reconcile(ctx, usage.Tokens{}); !errors.Is(err, tokenbudget.ErrUnavailable) {
		t.Fatalf("empty Reconcile = %v", err)
	}
	if err := nilHandle.Release(ctx, tokenbudget.ReleaseUnaccepted); !errors.Is(err, tokenbudget.ErrUnavailable) {
		t.Fatalf("nil Release = %v", err)
	}
	if err := (&durableReservation{repository: &BunTokenBudgetRepository{}}).Release(ctx, tokenbudget.ReleaseUnaccepted); !errors.Is(err, tokenbudget.ErrUnavailable) {
