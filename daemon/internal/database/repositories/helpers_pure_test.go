...
func TestMetadataHelpers(t *testing.T) {
	cases := []struct {
		...
		{observability.OutcomeSuccess, 200, ""},
		{observability.OutcomeInvalidReq, 400, string(observability.OutcomeInvalidReq)},
		{observability.OutcomeAuthFailed, 401, string(observability.OutcomeAuthFailed)},
		{observability.OutcomeQuota, 429, string(observability.OutcomeQuota)},
		{observability.OutcomeCancelled, 499, string(observability.OutcomeCancelled)},
		{observability.OutcomeUpstreamFail, 502, string(observability.OutcomeUpstreamFail)},
		{observability.OutcomeError, 502, string(observability.OutcomeError)},
		{observability.OutcomeUnavailable, 502, string(observability.OutcomeUnavailable)},
		{observability.Outcome("weird"), 502, "weird"},
	}
...
func TestTokenBudgetPureHelpers(t *testing.T) {
	...
	if !errors.Is(got, tokenbudget.ErrUnavailable) || !strings.Contains(got.Error(), "reserve persistence failed") {
