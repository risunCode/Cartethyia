...
func tokenReq() tokenbudget.ReservationRequest {
	return tokenbudget.ReservationRequest{
...
func TestTokenBudgetReserveLimitExceeded(t *testing.T) {
	...
	if !errors.Is(err, tokenbudget.ErrLimit) {
...
func TestTokenBudgetReleaseAndReconcile(t *testing.T) {
	...
	t.Run("release", func(t *testing.T) {
		...
		expectAnyExec(mock, nil)
		mock.ExpectCommit()
		if err := handle.Release(ctx, tokenbudget.ReleaseUnaccepted); err != nil {
			t.Fatal(err)
		}
	})
