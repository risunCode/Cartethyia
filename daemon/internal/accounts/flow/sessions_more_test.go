...
func TestManagerComprehensiveFlows(t *testing.T) {
	...
	// Start validations
	if _, err := m.Start("", accounts.FlowBrowser, "", "", "", 0); err == nil {
		t.Fatal("empty provider should fail")
	}
	if _, err := m.Start("p", accounts.OAuthFlowKind("invalid"), "", "", "", 0); err == nil {
		t.Fatal("invalid flow should fail")
	}

	// StartFromOAuth
	res := accounts.OAuthStartResult{
	...
	sOAuth, err := m.StartFromOAuth("openai", accounts.FlowBrowser, res)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := m.StartFromOAuth("", accounts.FlowBrowser, res); err == nil {
		t.Fatal("empty provider should fail")
	}
	...
	deniedS, _ := m.Start("openai", accounts.FlowBrowser, "", "", "", 0)
	...
	sCode, err := m.Start("claude", accounts.FlowBrowser, "", "", "", 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	// CompleteDevice
	sDev, err := m.Start("dev-provider", accounts.FlowDevice, "", "CODE", "https://verify", 5)
	if err != nil {
		t.Fatal(err)
	}
	...
	// Fail
	sFail, _ := m.Start("fail-provider", accounts.FlowBrowser, "", "", "", 0)
	...
	// Cancel
	sCancel, _ := m.Start("cancel-provider", accounts.FlowBrowser, "", "", "", 0)
