// Package drivers contains provider-specific OAuth lifecycle implementations.
...
func New(cfg Config) (*HTTPDriver, error) {
	...
	if cfg.Capabilities.AccessOnly {
		cfg.Kind = accounts.KindAccessOnly
	}
...
func (d *HTTPDriver) Kind() accounts.CredentialKind       { return d.cfg.Kind }
func (d *HTTPDriver) Capabilities() accounts.Capabilities { return d.cfg.Capabilities }

func (d *HTTPDriver) Start(ctx context.Context, input accounts.OAuthStartInput) (*accounts.OAuthStartResult, error) {
	flow := input.Flow
	if flow == "" {
		flow = accounts.FlowBrowser
	}
	if flow == accounts.FlowBrowser && !d.cfg.Capabilities.Browser {
		return nil, d.unsupported("browser flow")
	}
	if flow == accounts.FlowDevice && !d.cfg.Capabilities.Device {
	...
	if flow == accounts.FlowBrowser {
		...
		if err != nil || u.Scheme != "https" {
			return nil, d.wrap(accounts.ErrKindInvalidRequest, errors.New("authorization endpoint is not configured"))
		}
		q := u.Query()
		...
		d.mu.Unlock()
		return &accounts.OAuthStartResult{AuthorizationURL: u.String(), State: state, CodeVerifier: verifier, ExpiresAt: d.now().Add(10 * time.Minute), Flow: flow}, nil
	}
	...
	if deviceCode == "" {
		return nil, d.wrap(accounts.ErrKindInvalidRequest, errors.New("device response missing device code"))
	}
	...
	d.mu.Lock()
	d.states[state] = tokenState{providerID: d.cfg.ProviderID, deviceCode: deviceCode, expiresAt: d.now().Add(time.Duration(expires) * time.Second)}
	d.mu.Unlock()
	return &accounts.OAuthStartResult{State: state, UserCode: bounded(stringField(result, "user_code"), 128), VerificationURI: bounded(verification, 2048), IntervalSeconds: interval, ExpiresAt: d.now().Add(time.Duration(expires) * time.Second), Flow: flow}, nil
}

func (d *HTTPDriver) Poll(ctx context.Context, state string) (*accounts.OAuthPollResult, error) {
	...
	d.mu.Unlock()
	if !ok || st.deviceCode == "" {
		return nil, d.wrap(accounts.ErrKindInvalidRequest, errors.New("device session not found"))
	}
	if !st.expiresAt.IsZero() && !st.expiresAt.After(d.now()) {
		return &accounts.OAuthPollResult{Status: accounts.PollExpired}, nil
	}
	form := url.Values{"grant_type": {"urn:ietf:params:oauth:grant-type:device_code"}, "device_code": {st.deviceCode}, "client_id": {d.cfg.ClientID}}
	if d.cfg.ClientSecret != nil && !d.cfg.ClientSecret.IsZero() {
	...
	ts, err := d.tokenSet(result.body, accounts.OriginOAuthDevice, false)
	if err != nil {
		return nil, err
	}
...
func (d *HTTPDriver) Exchange(ctx context.Context, input accounts.OAuthExchangeInput) (*accounts.TokenSet, error) {
	...
	if d.cfg.ProviderID == ProviderGrokBuild && (!ok || strings.TrimSpace(input.State) == "") {
		return nil, d.wrap(accounts.ErrKindInvalidRequest, errors.New("Grok browser session is missing or expired"))
	}
	if verifier == "" {
		return nil, d.wrap(accounts.ErrKindInvalidRequest, errors.New("PKCE verifier is required"))
	}
	...
	ts, err := d.tokenSet(result.body, accounts.OriginOAuth, d.cfg.Capabilities.Refresh && !d.cfg.Capabilities.AccessOnly)
	if err == nil {
...
func (d *HTTPDriver) Refresh(ctx context.Context, input accounts.RefreshTokenInput) (*accounts.TokenSet, error) {
	if !d.cfg.Capabilities.Refresh || d.cfg.Capabilities.AccessOnly {
		return nil, accounts.NewError(accounts.ErrKindReauthentication, d.cfg.ProviderID, input.AccountID, errors.New("provider credential is access-only"))
	}
	if input.RefreshToken == nil || input.RefreshToken.IsZero() {
		return nil, accounts.NewError(accounts.ErrKindReauthentication, d.cfg.ProviderID, input.AccountID, errors.New("refresh token is missing"))
	}
	...
	return d.tokenSetWithRefresh(result.body, accounts.OriginOAuthRefresh, false, input.RefreshToken)
}

func (d *HTTPDriver) Revoke(ctx context.Context, input accounts.RevokeTokenInput) error {
	if !d.cfg.Capabilities.Revoke {
		return d.unsupported("revoke")
	}
	if input.Token == nil || input.Token.IsZero() {
		return d.wrap(accounts.ErrKindInvalidRequest, errors.New("revoke token is required"))
	}
...
func (d *HTTPDriver) tokenSet(body map[string]any, origin accounts.CredentialOrigin, requireRefresh bool) (*accounts.TokenSet, error) {
	return d.tokenSetWithRefresh(body, origin, requireRefresh, nil)
}
func (d *HTTPDriver) tokenSetWithRefresh(body map[string]any, origin accounts.CredentialOrigin, requireRefresh bool, old *accounts.Secret) (*accounts.TokenSet, error) {
	access := firstString(body, "access_token", "accessToken", "token")
	if access == "" {
		return nil, d.wrap(accounts.ErrKindInvalidRequest, errors.New("token response missing access token"))
	}
	refresh := firstString(body, "refresh_token", "refreshToken")
	if refresh == "" && requireRefresh && old == nil {
		return nil, d.wrap(accounts.ErrKindInvalidRequest, errors.New("token response missing refresh token"))
	}
...
func (d *HTTPDriver) requestResult(ctx context.Context, method, endpoint, contentType string, body []byte) (responseResult, error) {
	if endpoint == "" {
		return responseResult{}, d.wrap(accounts.ErrKindInvalidRequest, errors.New("OAuth endpoint is not configured"))
	}
	ctx, cancel := context.WithTimeout(ctx, d.timeout)
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, method, endpoint, strings.NewReader(string(body)))
	if err != nil {
		return responseResult{}, d.wrap(accounts.ErrKindInvalidRequest, err)
	}
	...
	if err != nil {
		if errors.Is(ctx.Err(), context.DeadlineExceeded) {
			return responseResult{}, accounts.NewError(accounts.ErrKindRefreshTransient, d.cfg.ProviderID, "", context.DeadlineExceeded)
		}
		return responseResult{}, accounts.NewError(accounts.ErrKindRefreshTransient, d.cfg.ProviderID, "", errors.New("OAuth network request failed"))
	}
	defer resp.Body.Close()
	limited := io.LimitReader(resp.Body, d.maxBody+1)
	raw, err := io.ReadAll(limited)
	if err != nil {
		return responseResult{}, accounts.NewError(accounts.ErrKindUnknown, d.cfg.ProviderID, "", errors.New("OAuth response read failed"))
	}
	if int64(len(raw)) > d.maxBody {
		return responseResult{}, accounts.NewError(accounts.ErrKindUnknown, d.cfg.ProviderID, "", errors.New("OAuth response exceeded limit"))
	}
	out := map[string]any{}
	if len(raw) > 0 {
		if err := json.Unmarshal(raw, &out); err != nil && resp.StatusCode < 300 {
			return responseResult{}, accounts.NewError(accounts.ErrKindUnknown, d.cfg.ProviderID, "", errors.New("OAuth response malformed"))
		}
	}
...
func (d *HTTPDriver) httpError(status int, code string) error {
	...
	kind := accounts.ErrKindRefreshFatal
	if status >= 500 || status == 408 || status == 429 {
		kind = accounts.ErrKindRefreshTransient
	}
	if status == 400 || status == 422 {
		kind = accounts.ErrKindInvalidRequest
	}
	if status == 401 || status == 403 || strings.Contains(strings.ToLower(code), "invalid_grant") || strings.Contains(strings.ToLower(code), "revoked") {
		kind = accounts.ErrKindReauthentication
	}
	return accounts.NewProviderError(kind, d.cfg.ProviderID, "", rawCode, fmt.Errorf("OAuth provider rejected request (%d)", status))
}
func (d *HTTPDriver) unsupported(op string) error {
	return accounts.NewError(accounts.ErrKindInvalidRequest, d.cfg.ProviderID, "", fmt.Errorf("%s is unsupported", op))
}
func (d *HTTPDriver) wrap(kind accounts.ErrorKind, err error) error {
	return accounts.NewError(kind, d.cfg.ProviderID, "", err)
}
