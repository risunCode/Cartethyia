...
func TestKiroSocialStartAndExchange(t *testing.T) {
	...
	if _, err := d.StartSocial(context.Background(), "twitter", ""); accounts.Classify(err) != accounts.ErrKindInvalidRequest {
	...
	if !strings.Contains(start.AuthorizationURL, "idp=Google") || start.Flow != accounts.FlowBrowser {
		t.Fatalf("social start = %#v", start)
	}
	if _, err := d.Exchange(context.Background(), accounts.OAuthExchangeInput{State: start.State}); accounts.Classify(err) != accounts.ErrKindInvalidRequest {
		t.Fatalf("missing social code error = %v", err)
	}
	ts, err := d.Exchange(context.Background(), accounts.OAuthExchangeInput{State: start.State, Code: "social-code"})
	if err != nil {
		t.Fatal(err)
	}
	...
	ts.Close()
	if _, err := d.Exchange(context.Background(), accounts.OAuthExchangeInput{State: start.State, Code: "social-code"}); accounts.Classify(err) != accounts.ErrKindInvalidRequest {
...
func TestKiroAWSStartAndPollStatuses(t *testing.T) {
	...
	start, err := d.Start(context.Background(), accounts.OAuthStartInput{Flow: accounts.FlowDevice, AWSMode: "builder-id", AWSStartURL: " https://aws.test/custom "})
	if err != nil {
		t.Fatal(err)
	}
	if start.Flow != accounts.FlowDevice || start.UserCode != "ABCD" || start.IntervalSeconds != 2 {
		t.Fatalf("AWS start = %#v", start)
	}
	if _, err := d.Start(context.Background(), accounts.OAuthStartInput{Flow: accounts.FlowDevice, AWSMode: "invalid"}); accounts.Classify(err) != accounts.ErrKindInvalidRequest {
	...
	for _, status := range []string{"pending", "slow", "denied"} {
		...
		want := accounts.PollPending
		if status == "denied" {
			want = accounts.PollDenied
		}
		if got.Status != want {
	...
	if err != nil || complete.Status != accounts.PollCompleted || complete.TokenSet.ProviderAccountID != "arn:profile" {
		t.Fatalf("completed poll = %#v %v", complete, err)
	}
	complete.TokenSet.Close()
	if _, err := d.Poll(context.Background(), start.State); accounts.Classify(err) != accounts.ErrKindInvalidRequest {
