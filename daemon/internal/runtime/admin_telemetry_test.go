...
func TestAdminTelemetryWindowRespectsExplicitBounds(t *testing.T) {
	from, to, err := adminTelemetryWindow(admin.TelemetryQuery{From: "2026-08-13T10:00:00Z", To: "2026-08-13T11:30:00Z"}, fixedTelemetryNow())
	if err != nil {
		t.Fatal(err)
	}
...
func TestAdminTelemetryWindowPeriodShortcuts(t *testing.T) {
	...
	for _, tc := range tests {
		from, to, err := adminTelemetryWindow(admin.TelemetryQuery{Period: tc.period}, now)
		if err != nil {
	...
	from, _, err := adminTelemetryWindow(admin.TelemetryQuery{Period: "all"}, now)
	if err != nil {
		t.Fatal(err)
	}
	if !from.IsZero() {
...
func TestAdminTelemetryWindowRejectsInvalidBounds(t *testing.T) {
	if _, _, err := adminTelemetryWindow(admin.TelemetryQuery{From: "not-a-timestamp"}, fixedTelemetryNow()); err == nil {
		t.Fatal("invalid from accepted")
	}
	if _, _, err := adminTelemetryWindow(admin.TelemetryQuery{To: "2026-13-45T00:00:00Z"}, fixedTelemetryNow()); err == nil {
		t.Fatal("invalid to accepted")
	}
}

func TestAdminTelemetryWindowInvertedBoundsAreEmpty(t *testing.T) {
	from, to, err := adminTelemetryWindow(admin.TelemetryQuery{From: "2026-08-13T11:00:00Z", To: "2026-08-13T10:00:00Z"}, fixedTelemetryNow())
	if err != nil {
		t.Fatal(err)
	}
	if !from.Equal(to) {
...
func TestAdminTelemetryBucketAutoResolution(t *testing.T) {
	...
	for _, tc := range tests {
		if got := adminTelemetryBucket(admin.TelemetryQuery{}, now.Add(-tc.window), now); got != tc.want {
			t.Fatalf("auto bucket for %v = %q want %q", tc.window, got, tc.want)
		}
	}
	if got := adminTelemetryBucket(admin.TelemetryQuery{Bucket: "minute"}, now.Add(-24*time.Hour), now); got != "minute" {
...
func TestPostgresTelemetryAdminServiceOverview(t *testing.T) {
	...
	overview, err := service.Overview(context.Background(), admin.TelemetryQuery{Period: "24h"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostgresTelemetryAdminServiceEmptyDataIsNotAnError(t *testing.T) {
	...
	overview, err := service.Overview(context.Background(), admin.TelemetryQuery{})
	if err != nil || overview.Requests != 0 || overview.ByRoute != nil {
		t.Fatalf("overview = %#v err=%v", overview, err)
	}
	buckets, err := service.Requests(context.Background(), admin.TelemetryQuery{})
	if err != nil || len(buckets) != 0 {
		t.Fatalf("requests = %#v err=%v", buckets, err)
	}
	upstream, err := service.Upstream(context.Background(), admin.TelemetryQuery{})
...
func TestPostgresTelemetryAdminServiceRequestsAndErrorsBuckets(t *testing.T) {
	...
	service := newPostgresTelemetryAdminService(store)
	service.now = fixedTelemetryNow
	query := admin.TelemetryQuery{Period: "24h", Bucket: "hour", Limit: 10000, Surface: "client_action"}
...
func TestPostgresTelemetryAdminServiceUpstreamGroups(t *testing.T) {
	...
	service := newPostgresTelemetryAdminService(store)
	service.now = fixedTelemetryNow
	buckets, err := service.Upstream(context.Background(), admin.TelemetryQuery{GroupBy: "model"})
	if err != nil {
		t.Fatal(err)
	}
	if len(buckets) != 2 {
...
func TestPostgresTelemetryAdminServiceWrapsStoreErrors(t *testing.T) {
	...
	if _, err := service.Overview(context.Background(), admin.TelemetryQuery{}); err == nil {
...
func TestPostgresUsageAdminServiceUsage(t *testing.T) {
	...
	service.now = fixedTelemetryNow
	summary, err := service.Usage(context.Background(), admin.TelemetryQuery{Period: "7d"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostgresUsageAdminServiceUsageWithoutData(t *testing.T) {
	...
	summary, err := service.Usage(context.Background(), admin.TelemetryQuery{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostgresUsageAdminServiceClients(t *testing.T) {
	...
	service := newPostgresUsageAdminService(store)
	service.now = fixedTelemetryNow
	dist, err := service.Clients(context.Background(), admin.TelemetryQuery{})
	if err != nil {
		t.Fatal(err)
	}
