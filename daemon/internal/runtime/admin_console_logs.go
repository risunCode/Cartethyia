...
func (s *consoleEventSink) Emit(_ context.Context, event observability.RequestEvent) error {
	if s == nil || event.Stage != observability.StageTerminal {
		return nil
	}
	ts := event.EndedAt
	if ts.IsZero() {
...
type fanOutEventSink struct{ sinks []observability.EventSink }

func (f fanOutEventSink) Emit(ctx context.Context, event observability.RequestEvent) error {
	var firstErr error
...
func consoleLogLevel(outcome observability.Outcome) string {
	switch outcome {
	case observability.OutcomeSuccess, observability.OutcomeCancelled:
		return "info"
	case observability.OutcomeQuota, observability.OutcomeAuthFailed, observability.OutcomeInvalidReq:
		return "warn"
	default:
		return "error"
	}
...
func consoleLogScope(surface observability.Surface) string {
	if surface == "" {
		return "unknown"
	}
...
func consoleLogStatus(outcome observability.Outcome) int {
	switch outcome {
...
func consoleLogMessage(event observability.RequestEvent) string {
...
func (s *postgresConsoleLogAdminService) List(ctx context.Context, input admin.ConsoleLogQuery) ([]admin.ConsoleLogEntry, error) {
	from, fromErr := parseAdminTime(input.From)
	to, toErr := parseAdminTime(input.To)
	if fromErr != nil || toErr != nil {
		return nil, admin.NewError(admin.CodeAdminInvalidRequest, "invalid console log window: from/to must be RFC3339 timestamps")
	}
	...
	merged := make([]admin.ConsoleLogEntry, 0, len(rows))
...
func adminConsoleLogKeyOf(entry admin.ConsoleLogEntry) adminConsoleLogKey {
	return adminConsoleLogKey{timestamp: entry.Timestamp, level: entry.Level, scope: entry.Scope, message: entry.Message}
}

func consoleEntryFromRow(row models.ConsoleLog) admin.ConsoleLogEntry {
	return admin.ConsoleLogEntry{
...
func consoleEntryFromRecord(record consoleLogRecord) admin.ConsoleLogEntry {
	entry := admin.ConsoleLogEntry{
		ID:        "live-" + strconv.FormatUint(record.seq, 10),
		Timestamp: record.timestamp.UTC().Format(time.RFC3339),
		Event:     string(observability.StageTerminal),
		Level:     record.level,
		Scope:     record.scope,
...
// origin column.
func consoleRecordMatches(record consoleLogRecord, from, to time.Time, input admin.ConsoleLogQuery) bool {
...
var _ admin.ConsoleLogService = (*postgresConsoleLogAdminService)(nil)
var _ observability.EventSink = (*consoleEventSink)(nil)
