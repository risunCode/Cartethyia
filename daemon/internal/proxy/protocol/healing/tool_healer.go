// Package healing provides bounded in-place repair for same-surface
// protocol requests before they are sent upstream.
package healing
			...
			if tcs, ok := msgCopy["tool_calls"].([]any); ok && len(tcs) > 0 {
				...
				for j, tcRaw := range tcs {
					if tcMap, ok := tcRaw.(map[string]any); ok {
						tcCopy := jsonclone.CloneMap(tcMap)
