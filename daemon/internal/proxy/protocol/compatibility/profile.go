// Package compatibility owns bounded, non-authoritative client dialect evidence.
...
func NewClientProfile(id ClientProfileID, surface contracts.Surface, confidence uint8, reasons []string) (ClientProfile, error) {
...
// Request is a compatibility alias useful to ingress callers.
type Request = ClassificationInput
...
func Classify(input ClassificationInput) (ClientProfile, error) {
	...
	if surface == contracts.SurfaceGemini && bodyKnown && bodySurface == contracts.SurfaceGemini && hintID == "" && !hasAmbiguity(ambiguities, AmbiguityEndpointBody) {
		id = ProfileGeminiCLI
		confidence = 86
...
func ClassifyRequest(input Request) (ClientProfile, error) { return Classify(input) }

func ClassifyClientProfile(input ClassificationInput) (ClientProfile, error) {
	return Classify(input)
}

func ClassifySurface(surface contracts.Surface, endpoint string, body []byte, headers http.Header) (ClientProfile, error) {
...
func classifySurface(input ClassificationInput) (contracts.Surface, []Evidence, []Ambiguity) {
	...
	return contracts.SurfaceOpenAIChat, []Evidence{{Code: EvidenceStandardShape, Authoritative: false}}, []Ambiguity{{Code: AmbiguityUnsupportedSurface}}
}

func endpointSurface(endpoint string) contracts.Surface {
...
func classifyBody(body []byte) (contracts.Surface, []Evidence, bool) {
	...
	case len(shape.Contents) > 0:
		return contracts.SurfaceGemini, []Evidence{{Code: EvidenceBodySurface, Authoritative: true}}, true
	case len(shape.Messages) > 0 && len(shape.MaxTokens) > 0:
		return contracts.SurfaceAnthropic, []Evidence{{Code: EvidenceBodySurface, Authoritative: true}}, true
	case len(shape.Input) > 0:
		return contracts.SurfaceOpenAIResponses, []Evidence{{Code: EvidenceBodySurface, Authoritative: true}}, true
	case len(shape.Messages) > 0:
		return contracts.SurfaceOpenAIChat, []Evidence{{Code: EvidenceBodySurface, Authoritative: true}}, true
	default:
		return "", nil, false
	}
}

func headerHint(headers http.Header) (ClientProfileID, contracts.Surface, []Evidence) {
	...
	for _, candidate := range []struct {
		...
		{ua, ProfileClaudeCode, contracts.SurfaceAnthropic, EvidenceHeaderUserAgent},
		{xapp, ProfileClaudeCode, contracts.SurfaceAnthropic, EvidenceHeaderXApp},
		{ua, ProfileCodexCLI, contracts.SurfaceOpenAIResponses, EvidenceHeaderUserAgent},
		{xapp, ProfileCodexCLI, contracts.SurfaceOpenAIResponses, EvidenceHeaderXApp},
		{intent, ProfileCodexCLI, contracts.SurfaceOpenAIResponses, EvidenceHeaderIntent},
		{ua, ProfileGeminiCLI, contracts.SurfaceGemini, EvidenceHeaderUserAgent},
		{xapp, ProfileGeminiCLI, contracts.SurfaceGemini, EvidenceHeaderXApp},
		{ua, ProfileOpenAICompatible, contracts.SurfaceOpenAIChat, EvidenceHeaderUserAgent},
		{xapp, ProfileOpenAICompatible, contracts.SurfaceOpenAIChat, EvidenceHeaderXApp},
	} {
