...
func TestCompatibilityErrorsAndConstructors(t *testing.T) {
	...
	if _, err := NewClientProfile("bad", contracts.SurfaceOpenAIChat, 1, nil); CodeOf(err) != CodeInvalidProfile {
...
func TestProfileValidationAndContextHelpers(t *testing.T) {
	valid := ClientProfile{ID: ProfileCodexCLI, Surface: contracts.SurfaceOpenAIResponses, Confidence: 100}
	if err := valid.Validate(); err != nil {
		t.Fatalf("valid profile error = %v", err)
	}
	for _, invalid := range []ClientProfile{
		{ID: "bad", Surface: contracts.SurfaceOpenAIChat},
		{ID: ProfileCodexCLI, Surface: contracts.SurfaceOpenAIChat, Confidence: 101},
		{ID: ProfileCodexCLI, Surface: "bad"},
		{ID: ProfileCodexCLI, Surface: contracts.SurfaceOpenAIChat, Reasons: []string{"has space"}},
	} {
...
func TestPlanCacheAndPlannerPaths(t *testing.T) {
	...
	backend := runtimecache.NewMemory(runtimecache.MemoryConfig{})
