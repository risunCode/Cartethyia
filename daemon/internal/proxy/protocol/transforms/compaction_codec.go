...
func EncodeCompactionRequest(ctx context.Context, protocol contracts.Protocol, req *NormalizedRequest) (*EncoderResult, *TransformError) {
	if req == nil { return nil, newTransformError(CodeInvalidCompaction, "encode-compaction", string(protocol), "request", "request must not be nil", nil) }
	...
	if req.Operation.Compaction == nil { return nil, newTransformError(CodeInvalidCompaction, "encode-compaction", string(protocol), "operation.compaction", "compaction request is required", nil) }
	if req.Operation.Kind == OperationCompactV2 { return NewOpenAIResponsesCodec().Encode(ctx, req) }
	...
	if req.Operation.Compaction.Instructions != "" { payload["instructions"] = req.Operation.Compaction.Instructions }
	if len(req.Operation.Compaction.Tools) > 0 { payload["tools"] = encodeResponsesTools(req.Operation.Compaction.Tools) }
	if req.Operation.Compaction.PromptCacheKey != "" { payload["prompt_cache_key"] = req.Operation.Compaction.PromptCacheKey }
	if err := req.Operation.Compaction.Validate(); err != nil { return nil, err }
	if err := validateWirePayload(contracts.ProtocolOpenAIResponse, payload); err != nil { return nil, err }
...
func encodeCompactionV1ResponsesInput(request *CompactionRequest) []map[string]any {
	...
	for _, block := range request.Input {
		switch block.Type {
		case BlockCompactionTrigger: continue
		case BlockText: input = append(input, map[string]any{"type": "message", "role": "user", "content": []map[string]any{{"type": "input_text", "text": block.Text}}})
		case BlockImage: input = append(input, map[string]any{"type": "message", "role": "user", "content": []map[string]any{{"type": "input_image", "image_url": openAIImageURL(block.Image)}}})
		case BlockAudio:
			if block.Audio != nil { input = append(input, map[string]any{"type": "message", "role": "user", "content": []map[string]any{{"type": "input_audio", "audio_data": block.Audio.Value, "format": audioFormat(block.Audio.MIMEType)}}}) }
		case BlockFile, BlockDocument, BlockPDF:
			if media := contentMediaReference(block); media != nil { input = append(input, map[string]any{"type": "message", "role": "user", "content": []map[string]any{encodeResponsesFileReference(media)}}) }
		case BlockReasoning: input = append(input, encodeResponsesReasoningItem(block))
		case BlockToolUse: input = append(input, map[string]any{"type": "function_call", "call_id": block.ToolCallID, "name": block.ToolName, "arguments": block.ToolArguments})
		case BlockToolResult: input = append(input, map[string]any{"type": "function_call_output", "call_id": block.ToolCallID, "output": block.Text})
		default: if block.Raw != nil { input = append(input, cloneMap(block.Raw)) }
		}
	}
	return input
}
...
func BridgeCompactionRequest(request *CompactionRequest, target CompactionVersion) (*CompactionRequest, *TransformError) {
	if request == nil { return nil, newTransformError(CodeInvalidCompaction, "bridge-compaction", "", "request", "request must not be nil", nil) }
	if target != CompactionV1 && target != CompactionV2 { return nil, newTransformError(CodeInvalidCompaction, "bridge-compaction", "", "version", "unsupported target compaction version", nil) }
	copyRequest, err := NewCompactionRequest(*request)
	if err != nil { return nil, err }
	copyRequest.Version = target
	for i := range copyRequest.Input {
		if copyRequest.Input[i].Compaction != nil { copyRequest.Input[i].Compaction.Version = target }
		if target == CompactionV1 && copyRequest.Input[i].Type == BlockCompactionTrigger { copyRequest.Input = append(copyRequest.Input[:i], copyRequest.Input[i+1:]...); break }
	}
	if target == CompactionV2 {
		hasTrigger := false
		for _, item := range copyRequest.Input { if item.Type == BlockCompactionTrigger { hasTrigger = true; break } }
		if !hasTrigger { copyRequest.Input = append(copyRequest.Input, ContentBlock{Type: BlockCompactionTrigger, Compaction: &CompactionContent{Version: CompactionV2, Kind: CompactionItemTrigger}}) }
	}
	if validateErr := copyRequest.Validate(); validateErr != nil { return nil, validateErr }
	return copyRequest, nil
}

func encodeResponsesTools(tools []Tool) []map[string]any {
	out := make([]map[string]any, 0, len(tools))
	for _, tool := range tools { if tool.NativeType != "" { value := cloneMap(tool.NativeOptions); if value == nil { value = map[string]any{} }; value["type"] = tool.NativeType; out = append(out, value); continue }; out = append(out, map[string]any{"type": "function", "name": tool.Name, "description": nilIfEmpty(tool.Description), "parameters": tool.InputSchema}) }
	return out
}
