...
func TestLimiterBlocksAndReleasesExactlyOnce(t *testing.T) {
	l, err := New(Layer{Name: "global", Limit: 1}, Layer{Name: "key", Limit: 1})
	if err != nil {
		t.Fatal(err)
	}
...
func TestLimiterCloseWakesWaiter(t *testing.T) {
	l, _ := New(Layer{Name: "global", Limit: 1})
...
func TestWaiterLimitAndFairness(t *testing.T) {
	...
	time.Sleep(time.Millisecond)
	if _, err := l.Acquire(context.Background(), map[string]string{"global": "second"}); !errors.Is(err, ErrLimit) && CodeOf(err) != string(CodeWaiterLimit) {
		t.Fatalf("waiter limit err=%v code=%s", err, CodeOf(err))
	}
	first.Release()
	select {
	case lease := <-granted:
...
func TestBoundedWaiterTimeoutHasStableCode(t *testing.T) {
	...
	if got := CodeOf(err); got != string(CodeCanceled) {
