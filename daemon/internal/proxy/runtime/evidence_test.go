...
func TestRouterEmitsExactlyOneRecordPerTransportCall(t *testing.T) {
	...
	recorder := observability.NewRecorder(context.Background(), sink, observability.WithCapacity(16))
	...
	var attempts []observability.RequestEvent
	for _, event := range sink.Events() {
		if event.Stage == observability.StageProviderCall {
	...
	if attempts[0].AttemptResult != observability.AttemptFailed || attempts[1].AttemptResult != observability.AttemptSucceeded {
...
func (panickingAttemptObserver) ObserveAttempt(observability.AttemptEvidence) {
	panic(errors.New("observer panic"))
}
func (panickingAttemptObserver) ObserveCandidateExclusion(observability.CandidateExclusionEvidence) {
	panic(errors.New("observer panic"))
}
func (panickingAttemptObserver) ObserveRepair(observability.RepairEvidence) {
	panic(errors.New("observer panic"))
}
func (panickingAttemptObserver) ObserveStreamFinalization(observability.StreamFinalizationEvidence) {
	panic(errors.New("observer panic"))
}
