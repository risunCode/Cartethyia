...
func newStreamingLifecycleFixture(t *testing.T, events chan StreamEvent) (*DispatchService, *AccountPool, *admission.Limiter, *controlledStreamTransport) {
	t.Helper()
	...
	limiter, err := admission.New(
		admission.Layer{Name: "global", Limit: 1},
		admission.Layer{Name: "stream", Limit: 1},
	)
	if err != nil {
...
func assertLifecycleHeld(t *testing.T, pool *AccountPool, limiter *admission.Limiter, transport *controlledStreamTransport) {
	t.Helper()
...
func assertLifecycleReleased(t *testing.T, pool *AccountPool, limiter *admission.Limiter, transport *controlledStreamTransport) {
	t.Helper()
