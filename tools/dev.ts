...
const routerPort = process.env.CARTETHYIA_DEV_DAEMON_PORT ?? "12801";
const dashboardUrl = process.env.CARTETHYIA_DASHBOARD_URL ?? `http://127.0.0.1:${dashboardPort}/home`;
const routerHealthUrl = process.env.CARTETHYIA_HEALTH_URL ?? `http://127.0.0.1:${routerPort}/health`;
const localEnv = {
  ...process.env,
...
try {
  ...
  start("router", ["go", "run", "./cmd/cartethyia"], routerDir, {
    CARTETHYIA_LISTEN_ADDRESS: `:${routerPort}`,
  });
  await waitForHttp(routerHealthUrl, "router");

  start("dashboard", ["bun", "run", "dev", "--host", "127.0.0.1"], dashboardDir, {
    CARTETHYIA_DASHBOARD_PORT: dashboardPort,
    CARTETHYIA_DAEMON_PORT: routerPort,
  });
