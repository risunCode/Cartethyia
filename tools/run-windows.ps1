...
$routerDir = Join-Path $repo "router"
$binary = Join-Path $routerDir "tmp\cartethyia-windows.exe"
...
if (-not $SkipBuild) {
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Router build gagal." }
  } finally {
    Pop-Location
  }
}
...
if (-not (Test-Path $binary)) {
  throw "Router binary tidak ditemukan: $binary"
}
